...
- 项目通常使用顶层 `tests/` 组织跨包测试；`tests/runtime-api/` 是 JavaScript Runtime API Conformance Lab 的正式、唯一例外，禁止将它重命名或迁回 `tests/`。可复用 fixture 放入所属测试域；一次性运行结果写入 `.runtime/<domain>/`（Runtime Lab 固定为 `.runtime/tests/runtime-api/`），正式质量报告放入 `docs/quality/`，外部参考 manifest 放入 `docs/research/external/`。
