...
- 测试所用的接口路径、请求参数和返回数据格式应以 `docs-user-api/` 中的文档为准；`docs-api-user/`、`docs-api/` 和 `docs/api/` 已退役，禁止恢复。
...
- 项目不再创建顶层 `artifacts/`；可复用 fixture 放入所属的 `test/**/fixtures/` 或 `tests/**/fixtures/`，正式质量报告放入 `docs/quality/`，外部参考 manifest 放入 `docs/research/external/`；一次性运行结果统一写入 `.runtime/`。
