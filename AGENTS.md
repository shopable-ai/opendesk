...
- 自动化任务需要发现或调用框架能力时，从 `docs/api/agent/README.md` 进入相关能力目录，再取得选中方法的 canonical 正文及必要公共约束；普通能力发现和业务脚本开发不读取机器维护总表、全部类型或历史案例。已明确的方法可直接取得阅读包。发现入口不替代下述专项 Skill、授权、数据真实性与安全停止要求。
