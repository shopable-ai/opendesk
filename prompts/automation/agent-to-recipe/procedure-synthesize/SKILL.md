...
description: 根据 OpenDesk 任务合同、应用资料和真实示范，或固定既有脚本与有界事实资料，完成因果复盘、业务分段与参数化；缺证据时定向补采。
---
...
已有脚本接续可以在计划明确限定的范围内，用 hash 固定的脚本、合同和按实际作用标注的证据做有限反向提炼。代码只能直接支持实现结构；业务意图、现场行为与正确性仍需相应证据。若目标是完整新生成，上段的成功示范要求不变。
...
9. 反向提炼只覆盖计划声明的修复节点、数据关系或 criterion；把代码推断与已观察事实分开，范围外分支、泛化和恢复规则继续列 unresolved 或提出定向补采，不因已有脚本曾运行就补齐。
...
接续产物按共享合同记录有限反向提炼来源和边界，本 Skill 不另定义字段。若原样复用无需改变业务过程，可以不为凑齐链路而制造 SemanticProcedure；需要修复时只交付足以约束该修复的过程范围。

## Gate 与失败

每个保留步骤和关键数据关系应有依据；无合理来源的业务值、不确定副作用、缺失任务结果证据不得放行生成。受限接续 gate pass 只能供同范围的最小修复消费，不能表述为完整过程或完整新生成 Gate。资料／引用问题映射 F7；业务解释 F3；验证缺口 F6；风险 F9。补采请求返回责任 Skill，不改写原观察。
