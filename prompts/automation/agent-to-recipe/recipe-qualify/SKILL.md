...
冻结候选可以是新生成、最小修复或原样复用的已有脚本；来源不改变验收强度。历史／参考运行不能替代本次候选资格证据，接续也必须执行获准的 Fresh Run。
...
5. 按获准范围做不同输入、旧状态污染、支持范围内窗口移动、错误期望等反例。最小修复覆盖变更范围及计划声明的回归边界；原样复用也不能仅凭旧记录通过。测试应有界、低风险，不为了造失败撤销系统权限或破坏用户业务。
...
9. 最终资格只绑定本候选及范围。按共享合同如实记录链路来源以及请求、实际执行、已资格化和排除范围；不能缩小请求范围来取得 pass，也不能把接续或参考候选写成完整新生成来源。修改脚本或关键依赖后新建验收尝试；不回写上游事实，不静默推广平台支持。

## 必须保存的输出

`qualification.json` 按共享 QualificationRecord：candidateRef、contractRef、qualificationScope、scenarios、actualCommands、workingDirectories、executionRefs、buildProvenance、environmentScope、observedResults、evidenceRefs、failedCriteria、skipped、verdict、repairRequests。

最后发布 `handoff.json`。其 gate scope 不得宽于 QualificationRecord 的请求范围，gate 为 pass 时还不得宽于已资格化范围。区分“参考脚本 smoke 通过”“已有候选在声明范围内通过”“基本业务验证通过”“完整 Skill 链通过”，不能相互替代。`not-run` 不等于 pass；没有完整证据不填写 95 分以上等成绩。
