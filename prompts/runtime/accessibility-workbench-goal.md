...
复用同一个 OpenDesk 进程和 `pkg/http` 基础组件。默认提供 Inspector 专用的 loopback 来源和路由集合；不挂载通用 `/executions`、Scheduler 写入或任意文件接口。仅当现有监听器已具备等效认证及隔离并有测试时才同源复用，记录依据。不得让用户管理第二个 Node／Python 服务或重复 OpenDesk 实例。

新增入口通过真实 CLI／已有桌面工具菜单可信启用，不以浏览器请求自行授予权限。为用户确定一个明确的一行启动方式，随程序帮助可发现；正式实现前不要把拟议的 `-inspect`、URL 或菜单宣称已存在。程序已运行但能力未启用时，说明状态，不自动重启或打断其他执行。
...
对所有数据请求检查凭据、实际 loopback socket、配置中的 Host／端口及浏览器来源；明确 Origin 缺失时的同源 Fetch Metadata／Referer规则，不能简单放行，也不能错误地让正常同源 GET 全失败。拒绝跨站、null Origin、伪造 Host、跨会话、过期、撤销、重放与转发头绕过。CORS 不作为认证。非浏览器测试客户端只能采用明确的认证规则。

静态资产随包分发，无 CDN、无遥测、无任意外部内容。新模块路由设置 CSP／frame-ancestors／nosniff／no-referrer／no-store；UI 数据按纯文本显示。与既有其他 HTTP 页面共享来源前，检查其对凭据和能力的影响，不把同进程等同于同权限。
...
将页面和必要内置程序纳入维护者生成的发布产物。用户只需已构建 OpenDesk 与浏览器，不需要克隆仓库、安装前端开发工具、编译 native host 或取得网页附件。
...
| 分发 | 仅发布产物和浏览器的真实命令；没有 checkout／CDN／外部工具；确认主程序与静态资产为当前构建 |
