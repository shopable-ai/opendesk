...
1. **反向检查能否实现。** 逐个业务步骤确认目的、输入来源、运行时生产者与消费者、前后置、观察、停止及副作用；逐个操作确认规则、目标、实际 API、异步语义、环境和验证状态。业务关键含义缺失回 S8—S9，操作规则缺失回应用工程；不在代码里猜、补占位方法或静默选择新 fallback。工程待验证不等于已通过；仅在本次构建范围明确允许时形成带限制的候选，不能宣称生产可用。
