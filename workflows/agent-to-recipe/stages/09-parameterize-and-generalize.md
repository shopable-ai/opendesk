...
读取 S8 步骤草案、S7 分析、冻结合同、S6 的实际输入／运行时值和适用 AppProfile。不得把样本里的全部字面量直接升级为常量，也不把全部常量变成用户参数。
...
发布 `procedure.json`：业务 steps、参数与来源、dataDependencies、preconditions／postconditions、verifiers、supportedScope、unresolved、evidenceRefs 和 applicationGaps；关联 S7／S8 产物，最后发布 handoff。
...
变量来源清楚、真实值消费关系保留、支持范围与依据一致、没有影响正常路径的未决业务问题时通过。过程结构错误返回 S8，来源不清返回 S7／S6，业务合同变更返回 S1。
