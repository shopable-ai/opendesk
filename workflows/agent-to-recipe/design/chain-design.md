...
历史六个 Skill 的目录已在 `17ccb9258dd34ce8b7c21296339a17f0c46e6586` 删除。下表保留六项职责及拟新增 code-rebuild 的设计名称，不代表实现仍存在或可调用。正式 Skill、合同兼容与宿主加载均需后续核实、实施和验证，本轮不恢复已删除目录。
...
- 现有共享合同记录历史六项职责的调用与接续语义；独立 code-rebuild、超出 minimal-repair 的获准优化、人工开发来源及质量裁剪须先做兼容设计。不能悄悄新增 JSON 枚举或把代码改进伪称新示范。
