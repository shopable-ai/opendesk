...
从真实任务／人工开发目标／已有自动化资产出发，形成有依据、可独立运行、可验证并能维护的普通 OpenDesk JavaScript。状态：阶段性框架分析，不是最终 WORKFLOW.md，也不是已运行通过报告。返回[设计总纲](README.md)；贯穿案例见[计算器](../cases/calculator.md)。
...
- 应用操作细化见[应用操作建模与封装](application-operations.md)；S11 的按需质量改进见[code-rebuild](code-rebuild.md)。这些是专业设计，不声明对应新 API、自动调度或 Skill 已实现。
- 本文回答完整需要做什么；[requirements.md](requirements.md)明确来源和需求基线，[chain-design.md](chain-design.md)明确环节、输入输出和组合，[validation-plan.md](validation-plan.md)明确凭什么通过。确认后才生成正式 WORKFLOW 和多个 Skill。
...
- **真实任务／人工开发目标／已有自动化资产**
  - **Ⅰ. 从任务要求到可信执行依据**
    ...
    - **S2｜建立足以安全推进下一步的最小应用认识**
      ...
      - 只消除影响下一步的未知。
        - 有效知识直接复用，不重复研究整个软件；发现阶段不要求先有完整提炼过程。
...
- 复用[六个专业 Skill](../../../prompts/automation/agent-to-recipe/README.md)的现有基础，目标职责以[链路设计](chain-design.md)为准。
...
- 保持证据与设计记录可追溯。
  ...
  - 本轮仅文档完整性检查不构成任一业务阶段通过，不填写无证据高分或整链通过。
...
- 本次更新：明确设计而非最终运行入口；生成与独立可选改进分开，替代旧“改名／必经优化”决定；补充用途与风险裁剪、需求来源和实施前链路关系。未删除原阶段，也未修改上游阶段编号或当前 Skill 合同。
