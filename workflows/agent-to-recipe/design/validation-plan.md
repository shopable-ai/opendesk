...
状态：验证设计 v0.2，2026-09-07。本文件定义应怎样验证，不是已执行的质量报告。实际 Skill、宿主加载、桌面测试及评分均未因文档写入自动通过。返回[设计总纲](README.md)，需求见[requirements.md](requirements.md)，责任映射见[chain-design.md](chain-design.md)。
...
以下 BC 标识是本设计的行为／评估案例，不是新的 Gate 枚举。当前均为 planned／not-run，不能填写通过或虚构实际读数。
...
- **BC-15｜独立上下文与真实宿主加载**
  ...
  - 未实现的 code-rebuild 或未迁移目录不能写成已被宿主识别。
- **BC-16｜资料留存与证据失效**
...
- 本轮只做设计文件的结构、链接、内容保留和覆盖核对，不启动 Runtime、UI、构建、mock 或实际业务测试。
...
- 逐项核对 DREQ-01—DREQ-16 是否在[链路映射](chain-design.md)中对应责任、行为案例和成果。
- 核对 S1—S12、R1—R13、三个循环、三种入口、五个结果层次和计算器分段仍然完整；只改变文件位置不能丢掉方法。
- 核对本轮重点：Research 与 ADR 有界，Workflow／Skill 分工在实施前明确，生成和独立可选改进分离，实际宿主可用性不靠目录推断。
...
当前：设计材料写入与迁移核对属于文档工作；BC-01—BC-16 的实际执行、Skill 加载、独立上下文及计算器验收均尚未进行。本计划不能作为任何一项通过证据，后续由真实结果填入独立记录。
