...
| 能力导航 | docs/api/README.md；按方法问题进入 docs/frameworks/README.md | 从业务意图定位相关模块；不把框架路线图当已实现 API |
| 类／模块方法概览 | 对应 Reference 的“API 一览”，例如 desktop-ui.md、window.md、accessibility.md | 形成候选，避免只知低层工具或重复造轮子；不读取无关整页 |
...
`runtime-api.ai.json` 不作为默认必读整包。需要时只提取相关模块定位信息；`keyMethods` 不是完整方法证明，`ai schema` 也只覆盖专用 CLI。未检索到方法时先核对相关 Markdown 方法目录和契约，必要时查类型／注册点，再区分“资料遗漏”“入口受限”“尚未实现”；不能凭单次空搜索定论。

读取工具返回截断内容，或公共约束分布在其他章节／文件时，继续取得必要范围；仅一个 H2 片段不保证合同完整。当前步骤的参数、结果、权限、错误、等待／取消、副作用及必要依赖均已明确且无冲突，即停止展开；引用不等于递归必读。分层 Markdown 方法目录及提取工具由 docs/api 维护，本页只接线工作流，不建立竞争目录或另一套方法契约。
...
按“Markdown 导航 → 相关类的方法概览 → 选中方法的精确契约及公共约定
→ 必要关联类型”获取能力；仅有冲突、缺口或失败时深入实现与测试。
不整包加载 runtime-api.ai.json、全部 .d.ts 或全部 Reference。
不把空搜索、keyMethods 或 ai schema 未列出方法当作能力不存在。
