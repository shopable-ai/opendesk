...
```text
artifacts/external/wechatweb-ref-20260405/
```

原因：
- 与运行工件分离，避免后续算法修复时被覆盖。
- 便于标记其为“外部参考源”，不是 agent 推理输出。
- 已符合当前仓库已有的 `artifacts/external/` 约定。

### 0.3 并行模型冲突规避
- 不覆盖 `artifacts/external/wechatweb-ref-20260405/`。
...
- 本地冻结来源：
  - `artifacts/external/wechatweb-ref-20260405/demo/index.remote.html`
  - `artifacts/external/wechatweb-ref-20260405/demo/runtime-desktop-1440x960.html`
  - `artifacts/external/wechatweb-ref-20260405/demo/demo-desktop-1440x960.png`
  - `artifacts/external/wechatweb-ref-20260405/repo/src/App.vue`
...
| 2 | E19,E10,E12 | 建立 immutable external / mutable run / promoted golden 三层目录。 | 如果 external 与 run 混放，另一个模型并行写入会互相覆盖。 | 只分层不命名唯一 run-id 仍会冲突。 | 并行模型冲突规避规则缺失。 | 本仓库已有 artifacts/external 与 artifacts/runs 先例。 | 63.5 | 63.5 | 继续 | 攻击并行执行冲突 |
...
| 15 | E19,E09,E10 | golden sample 目录只保存 manifest/replay/failure/evidence-index，具体大文件引用 run bundle。 | 若 manifest 不记录 source hash / file path，证据链仍弱。 | 还没纳入外部 demo/runtime html/hash。 | source provenance 字段不足。 | artifacts/external/wechatweb-ref-20260405 已含 repo/demo/runtime DOM。 | 79.0 | 79.0 | 继续 | 攻击 provenance |
...
- source：`artifacts/external/wechatweb-ref-20260405/demo/demo-desktop-1440x960.png`
- run-id：独立唯一
- 先产出：
