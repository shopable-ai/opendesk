...
macOS 真机验收应以真实 OpenDesk 的开发者菜单、Recorder 或全局快捷键启动，并保留冻结 Snapshot、底部工具条与 HUD、`2`、`I`、Inspector 的 `Esc`、Session 的 `Esc` 和重新进入的实窗证据到 `.runtime/tests/desktop-measurement/macos/`。Windows Native、物理混合 DPI / 多显示器和未具备条件的真实 Recorder 场景必须明确记录为 `NOT_RUN`，不能由浏览器测试补齐。
