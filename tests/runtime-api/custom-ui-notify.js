...
assert(typeof ui.toast === 'function', 'toast is not registered');
assert(typeof ui.notify === 'function', 'legacy notify alias is not registered');
const capabilities = ui.getCapabilities();
assert(capabilities.enabled && capabilities.available, 'matching enabled native host required');
assert(capabilities.window && capabilities.window.toast === capabilities.window.notify, 'toast capability must mirror the native notification backend');
for (const options of [null, {}, {message:''}, {message:'x',timeoutMs:-1}, {message:'x',timeoutMs:0.5}, {message:'x',progress:{value:2}}, {message:'x',unknown:true}, {message:'x',position:{mode:'absolute',x:1}}, {message:'x',position:{mode:'auto',x:1}}]) {
  await expect(() => ui.toast(options), 'INVALID_SPEC');
}
let hint;
try {
  hint = await ui.toast({message:'初始步骤',timeoutMs:0});
  const first = await hint.getState();
  assert(first.visible && first.nativeWindowId > 0 && first.toast && first.toast.message === '初始步骤' && first.toast.closable, 'persistent native creation/readback');
  assert(first.notification && first.notification.message === first.toast.message, 'legacy notification state alias must remain available');
  if (System.getPlatformInfo().os === 'darwin') assert(first.bounds.width === 280, 'short toast did not use the minimum native width');
  ...
  const expanded = await hint.update({
    message:'这是一条足够长的提示内容，用于确认同一个原生 Toast 句柄会按平台字体扩展宽度，并且不会超过受控的最大宽度。'.repeat(2),
  ...
  if (System.getPlatformInfo().os === 'darwin') assert(expanded.state.bounds.width === 480, 'long toast did not stop at the maximum native width');
  const result = await hint.update({message:'第 2 步',caption:'',progress:{min:0,max:5,value:1}});
  assert(result.applied && result.state.toast.progress.value === 1 && result.state.nativeWindowId === first.nativeWindowId, 'in-place update');
  ...
  assert((await hint.getState()).toast.progress.value === 1, 'failed patch mutated state');
  ...
  const legacy = await ui.notify({message:'legacy compatibility',timeoutMs:0});
  assert((await legacy.getState()).notification.message === 'legacy compatibility', 'legacy ui.notify compatibility failed');
  await legacy.close();

  for (let i=0;i<20;i++){const transient=await ui.toast({message:'cleanup '+i,timeoutMs:0});await transient.close();}
  console.log('CUSTOM_UI_TOAST_PASS');
