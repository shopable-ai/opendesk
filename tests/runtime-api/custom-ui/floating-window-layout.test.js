(() => {
  ...
  test({
    ...
    equal(registry.icons.length, 161, 'toolbar icon registry must keep 161 reviewed entries');
    const names = registry.icons.map(icon => icon.name);
    equal(new Set(names).size, 161, 'toolbar icon registry contains duplicate names');
    const batches = [];
