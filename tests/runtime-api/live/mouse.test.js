(() => {
  ...
  test({ name: 'mouse.wheel reaches HTML wheel listener', tier: 'live', covers: ['mouse.wheel'] }, async () => {
    const { point } = RuntimeLive.target('color-swatch');
    await mouse.click(point.x, point.y);
    await RuntimeLive.waitForCount('click', 1);
    await RuntimeLive.reset();
    await mouse.wheel({ deltaY: 120, steps: 2, delay: 10 });
    const snapshot = await RuntimeLive.waitForCount('wheel', 1);
    assert(RuntimeLive.events(snapshot, 'wheel').length >= 1, JSON.stringify(snapshot));
  });
})();
