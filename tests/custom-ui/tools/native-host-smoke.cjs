...
function call(operation,payload,windowId='toolbar'){
  ...
  return new Promise((resolve,reject)=>{
    ...
    child.stdin.write(JSON.stringify({version:'1.12.0',kind:'request',requestId,sessionId:'smoke',windowId,operation,payload})+'\n');
  });
}
...
(async()=>{
  ...
  const greet=await hello;clearTimeout(helloTimer);assert.equal(greet.version,'1.12.0');
