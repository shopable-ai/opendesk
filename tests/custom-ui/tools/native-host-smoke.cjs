...
for (const name of publicNames) {
  assert.equal(Array.from(windowsRegistry[name].glyph || '').length, 1, `Windows icon ${name} must map to exactly one glyph`);
}
...
function call(operation,payload,windowId='toolbar'){
  ...
  return new Promise((resolve,reject)=>{
    ...
    child.stdin.write(JSON.stringify({version:'1.7.0',kind:'request',requestId,sessionId:'smoke',windowId,operation,payload})+'\n');
  });
}
...
const control={id:'progress',kind:'progress',label:'Progress',width:160,min:0,max:5,value:1,revision:1};
...
(async()=>{
  ...
  const greet=await hello;clearTimeout(helloTimer);assert.equal(greet.version,'1.7.0');
  ...
  const moved=await call('setBounds',{...tools.bounds,x:tools.bounds.x+40,y:tools.bounds.y+40},'toolbar');
  assert.notEqual(moved.bounds.x,tools.bounds.x);
