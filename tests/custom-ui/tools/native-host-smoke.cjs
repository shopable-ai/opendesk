...
function call(operation,payload,windowId='toolbar'){
  ...
  return new Promise((resolve,reject)=>{
    ...
    child.stdin.write(JSON.stringify({version:protocolVersion,kind:'request',requestId,sessionId:'smoke',windowId,operation,payload})+'\n');
  });
}
...
(async()=>{
  ...
  const greet=await hello;clearTimeout(helloTimer);assert.equal(greet.version,protocolVersion);
