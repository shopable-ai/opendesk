...
func (c *sessionCapture) Count() int { c.mu.Lock(); defer c.mu.Unlock(); return c.count }
func (c *sessionCapture) Capture(_ context.Context, target string) (CaptureFrame, error) {
	...
	return CaptureFrame{PNG: data.Bytes(), Snapshot: Snapshot{SampledAt: time.Date(2026, 9, 14, 10, 0, c.count, 0, time.UTC), Mapping: mapping}, Reference: Reference{Type: ReferenceWindowOuter, Bounds: bounds, Window: &WindowIdentity{ID: target, PID: 7, Title: "Fixture"}}, Targets: []TargetWindow{{ID: "target", Title: "Fixture", PID: 7}, {ID: "other", Title: "Other", PID: 8}}, SelectedTargetID: target, TargetConfirmed: true}, nil
}
...
func (c *sessionClipboard) Copy(v string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
type createCountingDriver struct {
	customui.Driver
	mu      sync.Mutex
	creates int
}

func (d *createCountingDriver) Create(ctx context.Context, sessionID string, spec customui.WindowSpec, sink func(customui.Event)) (customui.DriverWindow, error) {
	d.mu.Lock()
	d.creates++
	d.mu.Unlock()
	return d.Driver.Create(ctx, sessionID, spec, sink)
}
func (d *createCountingDriver) CreateCount() int { d.mu.Lock(); defer d.mu.Unlock(); return d.creates }
...
func TestServiceReentryUsesSameSessionSnapshotAndSurface(t *testing.T) {
	...
	window := first.currentWindow()
	if err := service.Open(ctx, "recorder-toolbar"); err != nil {
		t.Fatal(err)
	}
	if service.active != first || service.active.currentWindow() != window || capture.Count() != 1 || driver.CreateCount() != 1 {
		t.Fatalf("reentry changed lifecycle active=%p first=%p captures=%d creates=%d", service.active, first, capture.Count(), driver.CreateCount())
	}
	if err := service.Close(ctx); err != nil {
		t.Fatal(err)
	}
	if service.Counts() != (ServiceCounts{}) || base.ResourceCounts().Sinks != 0 {
		t.Fatal("measurement resources leaked")
	}
}
func TestRefreshMutatesSameSurfaceAndMovesBounds(t *testing.T) {
	base := customui.NewMemoryDriver()
	driver := &createCountingDriver{Driver: base}
	capture := &sessionCapture{}
	service, err := NewService(ServiceOptions{Driver: driver, Capture: capture, Clipboard: &sessionClipboard{}, BaseDir: t.TempDir()})
	...
	if a.currentWindow() != window || driver.CreateCount() != 1 || capture.Count() != 2 {
		t.Fatalf("refresh replaced surface same=%v creates=%d captures=%d", a.currentWindow() == window, driver.CreateCount(), capture.Count())
	}
	...
	if state.Bounds != want {
		t.Fatalf("bounds=%+v want=%+v", state.Bounds, want)
	}
	_ = service.Close(ctx)
}
func TestWindowReferenceRequiresConfirmationAndManualFallback(t *testing.T) {
	...
	if a.result != nil || !strings.Contains(a.status, "确认") {
		t.Fatalf("unconfirmed candidate yielded result=%+v status=%q", a.result, a.status)
	}
	...
	_ = a.handle(ctx, customui.Event{Type: "measurement.pointerup", Fields: map[string]any{"u": .4, "v": .6}})
	if a.reference.Type != ReferenceManualRegion {
		t.Fatalf("manual reference=%+v", a.reference)
	}
	_ = service.Close(ctx)
}
func TestPointRegionTwoPointAndSpacingUseCanonicalSnapshot(t *testing.T) {
	service, _, _, _ := newSessionService(t)
	ctx := context.Background()
	if err := service.Open(ctx, "product-menu"); err != nil {
		t.Fatal(err)
	...
	_ = a.handle(ctx, customui.Event{Type: "measurement.pointerup", Fields: map[string]any{"u": .8, "v": .8}})
...
func TestInspectorCopyAndSavePreserveThreeTiers(t *testing.T) {
	...
	if err != nil || len(files) != 3 {
		t.Fatalf("saved files=%d err=%v", len(files), err)
	}
	jsonFound := false
	for _, f := range files {
...
func TestOpenAndWaitEscapeCleansUpWithoutClipboardMutation(t *testing.T) {
	...
	if service.Counts().Sessions != 1 || capture.Count() != 1 {
		t.Fatalf("not open counts=%+v captures=%d", service.Counts(), capture.Count())
	}
	if err := service.active.handle(context.Background(), customui.Event{Type: "measurement.key", Fields: map[string]any{"key": "Escape"}}); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-done:
	...
	case <-time.After(time.Second):
		t.Fatal("OpenAndWait did not return")
	}
	if len(clipboard.writes) != 0 || service.Counts() != (ServiceCounts{}) {
		t.Fatal("escape changed clipboard or leaked")
	}
}
func TestCloseRestoresCapturedWindowAndReportsLimitedRecovery(t *testing.T) {
	for name, restoreErr := range map[string]error{"restored": nil, "limited": errors.New("window was closed")} {
		t.Run(name, func(t *testing.T) {
			capture := &sessionCapture{}
			restores := 0
			service, err := NewService(ServiceOptions{Driver: customui.NewMemoryDriver(), Clipboard: &sessionClipboard{}, BaseDir: t.TempDir(), Capture: captureFunc(func(ctx context.Context, target string) (CaptureFrame, error) {
				f, e := capture.Capture(ctx, target)
				f.Restore = func(context.Context) error { restores++; return restoreErr }
				return f, e
			})})
			if err != nil {
				t.Fatal(err)
			...
			if restores != 1 || service.Counts() != (ServiceCounts{}) {
				t.Fatalf("restores=%d counts=%+v", restores, service.Counts())
			}
