...
func measurementHTMLProduct(a *activeSession) string {
	...
	return `<main id="measurementRoot">` +
		...
		`<p id="measurementStatus" class="inspectorStatus">` + html.EscapeString(a.status) + `</p><p id="referenceInfo" class="inspectorMeta">` + html.EscapeString(measurementReferenceSummary(a)) + `</p><p id="snapInfo" class="inspectorMeta">` + html.EscapeString(snapSummary(a)) + `</p>` +
		...
		`<div class="row secondaryActions"><button id="referenceButton">选择局部参照</button><button id="reselectReference">重新选择窗口</button><button id="copyStructured">复制结构化数据</button></div>` +
