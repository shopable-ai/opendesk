...
func TestGenerateReplayScriptUsesVisionRelocationAndWindowAnchors(t *testing.T) {
	...
	script, err := GenerateReplayScript([]TraceEvent{
		{
			Timestamp: now,
			Stage:     "tap_digit_7",
			App:       "Calculator",
		...
		ReplayDir: ".runtime/runs/replay-001",
	})
	if err != nil {
	...
	for _, want := range []string{
		...
		"page.screenshot",
		"DesktopVision.parse",
		"model response screenshot SHA mismatch",
		"perception screenshot SHA mismatch",
		"targetRole",
		"targetText",
		"bboxDistance",
		"pointDistance",
		"REPLAY_PLAN.windowTitle",
		"REPLAY_PLAN.windowWidth",
		"REPLAY_PLAN.windowHeight",
		"step.target.bbox_norm",
		"step.target.center_window",
		"File.ensureDir",
		"screenPointWithinWindow",
		"localPointWithinBox",
		"activeWindow.x + localClickPoint.x",
		"candidate.localClickPoint",
		"Screen.getDisplays",
		"Screen.getVirtualBounds",
		"page.checkScreenshotPermissions",
		"ensureFreshCapture(capture)",
		"screenshot became stale before action",
		"active window changed after visual re-detection",
		"resolved click point escaped the active display bounds",
		"resolved click point escaped the candidate bounds",
		"target is not unique after re-detection",
		"await main();",
	} {
...
func TestGenerateReplayScriptRejectsActionWithoutTarget(t *testing.T) {
	_, err := GenerateReplayScript([]TraceEvent{
		{
			Stage:  "broken",
			App:    "Calculator",
			Window: WindowIdentity{Title: "Calculator"},
...
func TestGenerateReplayScriptPreservesDryRun(t *testing.T) {
	...
	script, err := GenerateReplayScript([]TraceEvent{{
		Stage:  "dry_run_digit",
		App:    "Calculator",
		Window: WindowIdentity{BoundsScreen: ScreenBBox{100, 80, 500, 380}},
		Model:  ModelRef{Provider: "openai", Model: "gpt-5.6-sol", PromptVersion: "ui-parser-v1"},
		Target: &target,
		Action: &ActionRecord{Type: "click", DryRun: true},
	}}, ScriptOptions{})
	if err != nil {
		t.Fatalf("generate script: %v", err)
	}
