...
type replayPlan struct {
	...
	WindowHeight   float64      `json:"windowHeight,omitempty"`
	ReplayDir      string       `json:"replayDir,omitempty"`
...
type replayStep struct {
	...
	Expected *Postcondition `json:"expected,omitempty"`
}

func GenerateReplayScript(events []TraceEvent, opts ScriptOptions) (string, error) {
	plan, err := buildReplayPlan(events, opts)
	if err != nil {
		return "", err
	}
	...
	if err != nil {
		return "", fmt.Errorf("marshal replay plan: %w", err)
	}

	return fmt.Sprintf(`const REPLAY_PLAN = %s;
...
func buildReplayPlan(events []TraceEvent, opts ScriptOptions) (replayPlan, error) {
	plan := replayPlan{
		ReplayDir:      strings.TrimSpace(opts.ReplayDir),
		VisionProvider: strings.TrimSpace(opts.VisionProvider),
		Provider:       strings.TrimSpace(opts.Provider),
		Model:          strings.TrimSpace(opts.Model),
		MinConfidence:  opts.MinConfidence,
	}
	if plan.ReplayDir == "" {
		plan.ReplayDir = ".runtime/replay"
	}
	if plan.MinConfidence == 0 {
		plan.MinConfidence = DefaultConfidenceThreshold
	}

	for _, event := range events {
		if plan.App == "" {
			plan.App = firstNonEmpty(event.App, appFromPerception(event.Perception))
		}
		if plan.WindowTitle == "" {
			plan.WindowTitle = firstNonEmpty(event.Window.Title, windowTitleFromPerception(event.Perception))
		}
		if plan.WindowWidth == 0 {
			plan.WindowWidth = windowWidth(event.Window, event.Perception)
		}
		if plan.WindowHeight == 0 {
			plan.WindowHeight = windowHeight(event.Window, event.Perception)
		}
		if plan.PromptVersion == "" {
			plan.PromptVersion = strings.TrimSpace(event.Model.PromptVersion)
		}
		if plan.Provider == "" {
			plan.Provider = strings.TrimSpace(event.Model.Provider)
		}
		if plan.Model == "" {
			plan.Model = strings.TrimSpace(event.Model.Model)
		}
		if event.Action == nil {
			continue
		}
		if event.Target == nil {
			return replayPlan{}, fmt.Errorf("trace event %q is missing target", event.Stage)
		}
		if needsTarget(*event.Action) && event.Target.BBoxNorm.Validate() != nil {
			return replayPlan{}, fmt.Errorf("trace event %q is missing a valid normalized target bbox", event.Stage)
		}

		stepID := strings.TrimSpace(event.Stage)
		if stepID == "" {
			stepID = fmt.Sprintf("step_%02d", len(plan.Steps)+1)
		}
		plan.Steps = append(plan.Steps, replayStep{
			Index:    len(plan.Steps) + 1,
			ID:       sanitizeStepID(stepID),
			Stage:    event.Stage,
			Action:   event.Action.Type,
			Button:   event.Action.Button,
			Text:     event.Action.Text,
			Shortcut: event.Action.Shortcut,
			DryRun:   event.Action.DryRun,
			Target:   *event.Target,
			Expected: event.ExpectedPostcondition,
		})
	}

	if len(plan.Steps) == 0 {
		return replayPlan{}, fmt.Errorf("no actionable trace events found")
	}
	if plan.Provider == "" || plan.Model == "" {
		return replayPlan{}, fmt.Errorf("replay requires an explicitly verified vision provider and model")
	}
	return plan, nil
}
...
func needsTarget(action ActionRecord) bool {
	switch strings.TrimSpace(action.Type) {
	case "click", "type", "focus":
		return true
	default:
		return false
	}
...
func appFromPerception(snapshot *Perception) string {
	if snapshot == nil {
		return ""
	}
	return snapshot.App
...
func windowTitleFromPerception(snapshot *Perception) string {
	if snapshot == nil {
		return ""
	}
	return snapshot.Window.Title
}

func windowWidth(identity WindowIdentity, snapshot *Perception) float64 {
	if width := identity.BoundsScreen.Width(); width > 0 {
		return width
	}
	if snapshot == nil {
		return 0
	}
	return snapshot.Window.BoundsScreen.Width()
}

func windowHeight(identity WindowIdentity, snapshot *Perception) float64 {
	if height := identity.BoundsScreen.Height(); height > 0 {
		return height
	}
	if snapshot == nil {
		return 0
	}
	return snapshot.Window.BoundsScreen.Height()
}
