...
func TestRecoverInterruptedUninstallRestoresCatalogContentAndData(t *testing.T) {
	...
	journal := transactionJournal{
		SchemaVersion: 1, Kind: transactionUninstall, InstallID: installed.Record.InstallID,
		Backup: ".rollback-uninstall-test", DataBackup: ".rollback-data-test", RemoveData: true,
		Phase: transactionStaged, PreviousRecord: &installed.Record,
	}
	if err := service.writeJournal(journal); err != nil {
		t.Fatal(err)
	}
...
func TestRecoverUninstallAfterCatalogRemovalFinishesCommit(t *testing.T) {
	...
	journal := transactionJournal{
		SchemaVersion: 1, Kind: transactionUninstall, InstallID: installed.Record.InstallID,
		Backup: ".rollback-uninstall-commit-test", DataBackup: ".rollback-data-commit-test", RemoveData: true,
		Phase: transactionDataMoved, PreviousRecord: &installed.Record,
	}
	target := filepath.Join(service.Roots.FlowRoot, installed.Record.InstallID)
	backup := filepath.Join(service.Roots.FlowRoot, journal.Backup)
	if err := os.Chmod(target, 0o700); err != nil {
...
func TestUninstallPreservesDataByDefaultAndSharedPublisherTrust(t *testing.T) {
	...
	run, err := service.AcquireRun(context.Background(), second.Record.InstallID)
	if err != nil {
