...
│   └── opendesk/                            # primary CLI/HTTP entry and same-package tests
...
├── docs/  docs/api/                    # engineering docs and sole user-API docs
...
| --- | --- | --- | --- |
| `AGENTS.md`, `README.md`, `QUICKSTART.md`, `SUPPORT.md` | Repository rules, human entrypoints, and support/customization guidance. `QUICKSTART.md` was the one live mismatch: its build example left a binary at root. `SUPPORT.md` directs users to maintained API docs. | Keep at root. Change the build example to `make build` / `dist/opendesk`; do not duplicate API contracts in support material. | New users, support requests, `Makefile`, release scripts; no runtime code. |
| `Makefile`, `.gitignore` | The Makefile is the supported build/test command surface; `.gitignore` enforces the lifecycle boundary for `.runtime/`, `dist/`, `.staging-sync/`, local assistant state, and the accidental root `opendesk` binary. | Keep both at root. Build the primary command as `./cmd/opendesk`; keep local artifacts ignored rather than relocating them into source. | README/quickstart commands, build/package scripts, Git status and release output. |
...
| `automation/`, `pkg/`, `cmd/` | Application primitives, reusable packages, and command entrypoints. The formal baseline owns the primary executable in `cmd/opendesk/`; auxiliary commands remain separate siblings. | Keep these boundaries. Move the current primary command and its same-package tests together to `cmd/opendesk/`, retaining their local content; do not merge implementation from the disconnected remote history. | Go imports, `go build ./cmd/opendesk`, command-specific tests and scripts. |
| `docs/`, `docs/api/` | Engineering docs and the sole maintained user API root. `docs/` already has only `README.md` at its direct root; API documents have their own canonical root. The stale 2026-06 local-sync scope record had no incoming links and contradicted the current remote baseline. | Keep the two roots separate; archive that record under `.archive/reports/`; do not recreate `docs-api/` or `docs-api-user/`. | README links, user API publication, Runtime API checks. |
...
| `tests/` and Go `_test.go` files | Cross-package JS/fixture/replay domains live under `tests/`; JavaScript Runtime public contracts live under `tests/runtime-api/`. Pure Go/native white-box tests live beside the Go package they exercise because they need package-private access. The primary-command white-box tests now live beside `cmd/opendesk/main.go`; the dated WeChat progressive-recognition results document was a historical report, not current test guidance. | Keep `tests/` as the sole top-level test root; do not create a root `test/`. Keep primary command tests in `cmd/opendesk/`, not at root; move the historical report to `.archive/reports/wechat-layout/`. Delete or replace Go files that only duplicate a JS-observable Runtime contract; do not mechanically move package-private tests. | `scripts/test_runtime_apis.js`, `go test ./cmd/opendesk`, package-private test access. |
...
| `polyfills/`, `jslibs/`, `public/`, `types/` | Runtime loader loads polyfills and JS libraries; app packaging copies them. `public/` holds shipped media/static files. `types/` plus `jsconfig.json` supplies editor signatures. | Keep as distinct runtime assets/contracts; do not merge into docs or examples. | `cmd/opendesk/main.go` asset lookup, macOS app packaging, `docs/api/`, TypeScript tooling. |
...
| `dist/` and root `opendesk` | `dist/` is ignored, rebuildable release cache. A current 42 MB root `opendesk` executable was produced by the obsolete quickstart instruction and differs from the older `dist/opendesk`. | Keep `dist/`; move the root executable into `dist/opendesk` after recording its hash. Ignore `/opendesk` so it cannot be committed again. | `Makefile`, macOS build/open/run scripts, quickstart. |
...
The formal target is `cmd/opendesk/main.go`, not root `main.go`. Latest
`origin/master` already has `cmd/opendesk/main.go`, its command-local
`main_*_test.go` files, a `Makefile` that builds `./cmd/opendesk`, and packaging,
...
`cmd/opendesk/`: `main.go` and its three same-package tests. The local and
...
tree. This working tree must run `go build ./cmd/opendesk` and `go test
./cmd/opendesk` now.
