...
Agent 日常能力发现从 [Markdown 短入口](agent/README.md) 开始，完整行为回到选中方法的 canonical Reference，精确类型按需读取 `types/*.d.ts`；机器维护产物不进入正常业务阅读链。Runtime 内部注入与 polyfill 组成见 [Runtime API composition](../implementation/runtime/runtime-api-composition.md)，文档与类型同步规则见 [docs/api editme-cli TOC maintenance](../maintenance/docs-user-api-editme-toc-maintenance.md)。
