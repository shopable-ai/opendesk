...
11. `events.md`：外部窗口、应用、剪贴板与显示器状态变化 watcher
12. `app.md`：按 stable identity 启动、等待、终止与重启桌面应用
13. `vision.md`：OCR、UI 文本识别、provider
14. `image-color.md`：模板匹配、颜色与图像辅助能力
15. `screen.md`：显示器、像素、截图别名、区域选择与录屏
16. `audio.md`：系统音频控制、设备发现与 capability-gated 固定声音模式匹配
17. `sound.md`：提示音和本地音频播放
18. `notify.md`：发送系统通知
19. `notifications.md`：观察、等待与移除 OpenDesk 自身通知
20. `dialog.md`：异步 alert / confirm / prompt
21. `clipboard.md`：系统剪贴板
22. `global-apis.md`：无需 import 的全局接口、console、等待、计时器和参数工具
23. `sqlite.md`：第一方本地 SQLite
24. `environment.md`：环境变量、`.env`、输出配置与优先级
25. `execution.md`：Execution ID、结构化输入、工作目录、来源和 artifact 上下文
26. `path.md`：平台原生路径字符串处理
27. `runtime.md`：JavaScript 执行、异步生命周期与兼容边界
28. `command.md`：本地命令执行、输出、错误与 execution-owned 清理
29. `custom-ui.md`：OpenDesk 自己的 Dialog、FloatingWindow 与受限 HTML/CSS 原生窗口
30. `native-extension.md`：Native Extension Plugin V1
31. `cookbook.md`：可直接改造的脚本范例
32. `scheduler.md`：Scheduler 功能、生命周期、持久化与本地管理页
33. `scheduler-api.md`：Scheduler 独立 HTTP 协议契约
34. 其余专题页按需查阅

## 文档分层

- **核心桌面自动化**：`page.md`、`geometry.md`、`desktop-ui.md`、`mouse.md`、`input.md`、`window.md`、`screen.md`、`accessibility.md`、`global-shortcut.md`、`events.md`、`app.md`
