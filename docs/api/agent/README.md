...
普通 Agent 能力发现、Recipe 开发和局部维修只使用“本页 → 相关能力目录 → 选中方法 canonical Reference → 必要类型”这条阅读链；机器维护文件不作为备用能力入口。`.d.ts` 只按需核对，不替代行为契约。设计、历史案例和专项 Skill 只由当前缺口/任务路由触发，原授权与安全停止要求不变。
