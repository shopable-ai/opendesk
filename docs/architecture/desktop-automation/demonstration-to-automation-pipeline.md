...
1. 接住任务，先定义“什么结果才算真正完成”。
2. 进入应用，确认任务起点并建立初始界面认识。
3. 以完成业务为先，真实执行并同步形成有理由、有结果的操作经验。
4. 独立确认最终业务结果，封存一次完整且可追溯的示范。
5. 回看全过程，还原“刚才到底完成了什么”。
6. 区分正式路径、探索、错误、重试和恢复，提取因果成功路径。
7. 把细碎操作压缩成可审阅的业务步骤与状态转换。
8. 从一次示范归纳可复用的参数、Skill、Workflow 和控制逻辑。
9. 为每个业务步骤补齐稳定定位、等待、验证、失败处理和有限恢复。
10. 构造统一自动化模型，生成第一版程序并重构到接近人工质量。
11. 在新运行中真实回放、独立验证、扰动测试、修复和晋级。
12. 投入运行后持续监控漂移、错误目标、假成功与成本，版本化维护并在必要时重新学习。
...
### 阶段 6：区分正式路径、探索、错误、重试和恢复，提取因果成功路径
...
### 阶段 8：从一次示范归纳可复用的参数、Skill、Workflow 和控制逻辑
...
### 阶段 9：为每个业务步骤补齐稳定定位、等待、验证、失败处理和有限恢复
...
### 阶段 10：构造统一自动化模型，生成第一版程序并重构到接近人工质量

正式知识先进入 Canonical Automation IR，再由编译器派生 JavaScript、Skill 或 Playbook。第一版代码可以只达到“正确可回放”，但在晋级前必须完成业务化重构。
...
### 阶段 11：在新运行中真实回放、独立验证、扰动测试、修复和晋级
