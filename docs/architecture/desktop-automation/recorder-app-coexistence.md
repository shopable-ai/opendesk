...
Recorder UI 的唯一正式 JavaScript / 图标源码归属于内部 Recorder bundle：

```text
internal/recorderbundle/ui/
├── controller.js
├── controller-core.js
...
- `internal/recorderbundle/ui/**` 是 Recorder UI 的 canonical source。
...
- `internal/recorderbundle/bundle.go` 只负责 Go Runtime 的 embed 与 materialization；同目录 `ui/**` 才是 Recorder UI 的唯一实现。
...
| Recorder UI source | `internal/recorderbundle/ui/**` 为唯一 canonical copy |
