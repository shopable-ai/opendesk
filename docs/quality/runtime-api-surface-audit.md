...
| Audio | `automation/audio.go`、`audio_pattern_runtime.go` + platform backends | `registerAudio` 显式注册控制方法，并把 `watchSound/waitForSound` 接入 execution-owned matcher owner | `docs/api/audio.md` / `types/Audio.d.ts` | `unit/audio.test.js` + `seams/audio-pattern-positive.js`、`seams/audio-pattern-cleanup-failure.js`、`seams/audio-pattern-teardown.js` | 控制为同步；pattern watcher 的 capture、matcher、Promise、Close/Wait/ResourceCounts 进入 lifecycle；默认 capture backend fail closed | 契约与 lifecycle 闭合；真实平台 capture backend 尚未实现 |
