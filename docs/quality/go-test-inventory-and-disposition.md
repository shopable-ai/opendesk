...
| --- | ---: | --- | --- |
| 当前已跟踪源码 | 136 | 是，按 Go package 或嵌套模块分别处理 | 保留在所属 package，逐文件审查测试边界 |
| `.archive/batch1-sync-blockers-20260608-191116/` | 8 | 否 | 继续隔离；不恢复、不加入 gate、不直接删除 |
| 根模块当前 package | 132 | 是 | `go test ./...` 应作为实现回归 gate |
| `third_party/` 嵌套上游模块 | 4 | 根 gate 不自动覆盖 | 保留上游位置；单独、隔离、按平台运行 |

因此，“144 个都要移动到 `tests/`”不是正确方案。移动 package-private 测试会失去对
未导出函数、内部状态机和 fake backend 的访问，还会改变 Go package 的测试语义。

## 按目录的全量处置结论

下表覆盖本次 144 个文件的目录集合；同一行中的 glob 覆盖该目录下全部对应测试文件。

| 文件范围 | 数量 | 测试性质 | 处置方案 |
| --- | ---: | --- | --- |
| `automation/*_test.go` | 49 | Runtime native binding、backend、权限、生命周期、图像算法和私有 helper | 保留同包；JS 公共行为由 `tests/runtime-api/` 补充，不机械搬迁 |
...
| `cmd/opendesk/*_test.go` | 8 | CLI source、HTTP、配置、实例和脚本生命周期 | 保留 `cmd/opendesk`，因为大量测试访问 `package main` 内部函数 |
...
| 文件 | 当前问题 | 建议方案 |
| --- | --- | --- |
| `automation/image_layout_validation_test.go` | 主要生成带 ground truth 的 PNG/JSON，没有产品行为断言 | 迁移为 `tests/automation/tools/image-layout-fixtures/` 生成器；算法断言继续留在 `image_layout_test.go` / `image_layout_progressive_test.go` |
| `automation/image_layout_visualize_test.go` | 读取 fixture、运行分析并输出标注图，职责是可视化工具 | 迁移为 `tests/automation/tools/image-layout-visualize/`，输出只写 `.runtime/tests/` |
| `automation/wechat_visualization_test.go` | 依赖预先生成的 WeChat screenshot，主要生成分析图；已有 `tests/wechat/tools/` 工具集合 | 与对应 WeChat tool 对照合并，避免同一可视化逻辑同时存在于 package test 和 tool |
...
需要：明确命令、明确工作目录、默认输出 `.runtime/`，并在本目录或相关质量文档登记。
...
后续逐个处理 136 个当前源码测试文件时，每个文件都记录以下字段；不允许只根据文件名
移动或删除：
