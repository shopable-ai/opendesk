...
| `pkg/execution/audio_pattern_runtime_api_test.go` | `SPLIT_JS_CONTRACT` | 否：只用 exported execution Request/Run 和 Audio capture backend interfaces | 注入确定性 PCM 后运行真实 watchSound、waitForSound、3s/12s 双 order cue + payment/confuser 与 cleanup-failure JavaScript | Goja execution、合成 WAV、memory capture backend、临时 Evidence | 防止公开 match envelope、cooldown/干扰误触发、cached wait Promise、stop 幂等、cleanup failure 误 resolve 和资源计数断裂 | Go 仅提供 backend/harness；用户行为事实源是 `tests/runtime-api/seams/audio-pattern-positive.js`、`audio-pattern-market.js` 与 `audio-pattern-cleanup-failure.js`。 |
...
本节只追加基线之后新增的 Go 测试资产，不回写上方历史数量或迁移基线。

| 文件 | 主处置 | 私有依赖 | 主要职责 | 外部依赖 | 断言价值 | 结论与证据 |
| --- | --- | --- | --- | --- | --- | --- |
| `automation/accessibility_runtime_limits_test.go` | `SPLIT_JS_CONTRACT` | 是：`AccessibilityRuntime` 的 ref reservation、bounded queue、固定 native worker、resource counters 与 backend factory seam | 真实 Goja/EventLoop 下的 256 active refs、32 queued requests、Close 取消、迟到 handle 释放、未 await Promise teardown、release/stale 状态机 | 纯内存 fake App/Accessibility backend、goroutine 与 clock；不读取或操作真实桌面 | 防止超限请求进入 native backend、queued/in-flight teardown 泄漏、backend 跨 owner 调用、重复 release 或 stale ref 复活 | Go 文件只证明无法由公开 JS 稳定注入的 native owner/资源 seam；公开 Promise、错误 envelope 与默认生命周期继续由 `tests/runtime-api/accessibility.js`、`tests/runtime-api/accessibility-lifecycle.js` 负责。 |
| `automation/app_name_test.go` | `KEEP_PACKAGE` | 是：应用 target 的未导出 kind、macOS 规范化、匹配和 projection helper | macOS 本地化 Calculator 别名与进程观测身份的一致转换 | 纯内存 app target/state | 防止别名误匹配任意应用或 bundle ID、PID、观测名称投影漂移 | native App target 解析白盒；公开 `App` 结果由 `tests/runtime-api/acceptance/app-launch-name.js` 覆盖。 |
| `automation/console_routing_test.go` | `KEEP_PACKAGE` | 是：`initEventSink`、`consoleRoutingCapture` 与未导出 event 分类字段 | bootstrap console 事件降级及业务 console 方法保真 | 内存 sink | 防止框架 bootstrap 被伪装成业务日志或 console method 丢失 | Runtime 初始化日志路由 seam，不能由公开脚本构造相同的 bootstrap sink。 |
| `automation/file_json_test.go` | `SPLIT_JS_CONTRACT` | 是：文件提交状态机、写入 hooks、worker 生命周期与 RuntimeLifecycle | JSON 文件原子替换、取消时序、资源计数和无 EventLoop 拒绝 | 临时文件、符号链接、Goja EventLoop 和受控 worker | 防止失败覆盖旧文件、临时文件泄漏或 teardown 遗留异步 worker | native File JSON owner seam；公开 `File.readJSON()` / `File.writeJSON()` 契约由 `tests/runtime-api/unit/file-json.test.js`、`tests/runtime-api/acceptance/file-json.js` 覆盖。 |
| `automation/image_diff_test.go` | `KEEP_PACKAGE` | 是：image diff 解码、像素比较、忽略区域和 PNG 输出私有 helper | 合成图像下的阈值、alpha、裁剪、路径/data URL 输入与确定性差异图 | 合成 PNG、临时文件 | 防止像素边界、误差统计、忽略区域并集或输出编码发生漂移 | 图像算法白盒；用户层 `Image` API 的请求形状仍由 `tests/runtime-api/unit/image-color.test.js` 覆盖。 |
| `automation/image_template_benchmark_test.go` | `KEEP_PACKAGE` | 是：canonical matcher、template scene、region 和候选选择私有实现 | 典型桌面尺寸与模板尺寸下的纯 Go matcher 基准 | 合成内存图像 | 防止 benchmark 失去有效匹配而把无意义成本当性能数据 | package-local 性能回归基线，直接依赖未导出 matcher；不作为 Runtime 公共契约。 |
| `automation/image_template_opencv_benchmark_test.go` | `KEEP_PACKAGE` | 是：build-tagged experimental OpenCV primitive 与 canonical matcher 私有函数 | `opencv` tag 下 canonical 与实验性 TM_CCOEFF_NORMED 对照基准 | build-tagged OpenCV 依赖、合成图像 | 防止实验路径无匹配却被错误比较，且保持其非默认边界 | 可选 native 性能 seam，不能移为普通 JS Runtime 测试或默认目标系统验证。 |
| `automation/image_template_test.go` | `KEEP_PACKAGE` | 是：模板匹配私有 candidate、scale、region 与解码 helper | 单/多结果、阈值、缩放、区域、输入格式和结果去重 | 合成 PNG/JPEG/base64、临时文件 | 防止候选排序、阈值和 legacy `FindPos` 结果形状回归 | 图像匹配算法白盒；公开 Image capability 由 `tests/runtime-api/unit/image-color.test.js` 单独约束。 |
| `automation/sqlite_internal_test.go` | `SPLIT_JS_CONTRACT` | 是：SQLite DSN、SQL lexical guard、canonical path key 等 native helper | URI escaping、Windows/UNC fidelity、SQL 安全词法判断与 symlink parent identity | 临时文件、符号链接、注入 OS 名称 | 防止 native I/O 之前的路径混淆或 lexical guard 绕过 | native SQLite owner seam；脚本可观察的 API 由 `tests/runtime-api/unit/sqlite.test.js`、`tests/runtime-api/sqlite-contract.js` 覆盖。 |
| `cmd/opendesk/console_config_test.go` | `KEEP_PACKAGE` | 是：main package 的 flag 访问、console resolver 和 test-only environment helper | 文件、进程环境和命令行的 console 配置优先级 | 临时 `.env` 文件、in-memory environment | 防止 `--console-mode`、颜色策略、agent 输出和 invalid value 解析回归 | CLI 装配白盒，需直接调用未导出的 main resolver。 |
| `cmd/opendesk/launchservices_args_test.go` | `KEEP_PACKAGE` | 是：main package 的 `stripLeadingMacOSLaunchServicesPSN` helper | Finder/LaunchServices 注入的首个 PSN 参数移除边界 | 固定 argv vectors | 防止脚本参数或 `--` 后数据被错误删除 | macOS launch argument sanitizer 的私有 CLI seam。 |
| `pkg/customui/placement_test.go` | `KEEP_PACKAGE` | 是：memory window、placement resolver 和 work-area overflow helper | 九宫格锚点、初始约束、动态 placement 与负坐标 work area | 内存 window、固定 display geometry | 防止无效 placement 触达 host 或窗口落到不可见/overflow 坐标 | Custom UI driver 白盒；真实窗口视觉由独立 native UI 验收负责。 |
| `pkg/customui/toolbar_image_test.go` | `KEEP_PACKAGE` | 是：toolbar image validator、wire payload normalizer 与私有 image loader | 图标 metadata、输入路径、尺寸/编码限制和 source-path 隔离 | 受控 PNG/JPEG、临时文件 | 防止伪造 raster metadata、危险输入或本地 source path 泄露到 UI payload | Custom UI serialization/security seam，需同包 helper 与 toolbar model。 |
| `pkg/execution/audio_pattern_fixtures_runtime_api_test.go` | `SPLIT_JS_CONTRACT` | 是：execution `Run`、artifact lifecycle、注入 audio capture backend 与 cleanup parser | 真实 Goja execution 的 fixture audio start/stop、callback 与 artifact cleanup | 内存 PCM backend、临时 fixtures、goroutine 与 clock | 防止 native audio session 遗留、fixture timing 或 runtime cleanup 失真 | execution/native owner seam；脚本 API 行为由 `tests/runtime-api/sound-cancel.js`、`tests/runtime-api/sound-cancel-validation.js` 覆盖。 |
| `pkg/execution/emitter_test.go` | `KEEP_PACKAGE` | 是：Emitter writers、event finalization 和 agent summary 私有状态 | terminal ANSI 仅写终端、事件格式、finalize 去重和 status level | 内存 buffers、临时 artifact files | 防止 ANSI 污染日志、重复 execution error 或 terminal summary 级别漂移 | execution emitter 白盒，外部脚本不应直接控制其 writer/summary state。 |
| `pkg/execution/file_json_test.go` | `SPLIT_JS_CONTRACT` | 是：execution `Run`、artifact construction与未 await Promise drain | 每 execution workdir 隔离及 JSON 异步写入 teardown | 临时 workdir/artifacts、Goja Runtime | 防止相对路径跨 execution 或未 await 写入被遗弃 | execution lifecycle seam；公开 File JSON 语义由 `tests/runtime-api/unit/file-json.test.js` 覆盖。 |
| `pkg/execution/source_context_test.go` | `KEEP_PACKAGE` | 是：execution context 注册和 script path normalizer 私有 trust-boundary helper | SourceLabel 不能伪造 script path，relative script 只能解析到 immutable workdir | Goja Runtime、临时 workdir | 防止不可信 label 注入 path metadata 或脚本越出 workdir | execution provenance security seam，JS 无法构造受信 host Request。 |
| `pkg/terminalstyle/style_test.go` | `KEEP_PACKAGE` | 是：color policy、TTY detector、tag colorizer 与 summary parser 内部 helper | auto/always/never 颜色策略、prefix 着色、redirected stream 和 event-level priority | 内存 writers、注入 environment/TTY probe | 防止 ANSI 写入重定向流、颜色优先级错乱或 summary 从路径猜测失败 | terminal rendering package 白盒，非 Runtime public global。 |
| `tests/runtimeenv/environment_test.go` | `MOVE_GO_BLACKBOX` | 否：仅调用 exported `runtimeenv` Options、Resolve、Parse、Clone 与 Lookup | 项目 env 文件、显式文件、继承环境及平台名称规则 | 临时 `.env` 文件、固定 inherited environment | 防止 precedence、解析歧义、snapshot aliasing 或 Windows 名称规则漂移 | 外部 `runtimeenv_test` 包的黑盒测试，已位于顶层 `tests/runtimeenv/`，不依赖实现私有符号。 |
| `automation/recorder_internal_test.go` | `SPLIT_JS_CONTRACT` | 是：`recorderSession`、bounded queue、writer fault seam 与唯一动作归组函数 | 就绪后 stop 截止、积压排空、晚回调、重复 stop、取消、writer Write/Flush/Sync/Close/manifest 失败及动作边界 | 内存 backend、临时录制目录和脱敏事件 | 防止已接收事件在 stop 丢失、故障被伪装为 saved、drag/control-click/composition 被降级或键盘双重消费 | Go 文件只覆盖 JS 无法安全注入的 native owner/writer seam；公开 Recorder Promise、文件制作和 non-overwrite 契约由 `tests/runtime-api/recorder.js` 与 `tests/human-to-recipe/coordinate-recipe.js` 覆盖。 |
| `automation/recorder_uiohook_internal_test.go` | `KEEP_PACKAGE` | 是：libuiohook adapter 的进程级 atomic lease helper | dispatcher 独占、错误 owner 不可释放及 clean stop 后复用 | 纯内存 backend 指针；不启动系统监听 | 防止并发 execution 同时占用进程级 libuiohook dispatcher | build-tagged native 适配白盒，不产生真人输入副作用，也不替代 macOS/Windows live Runtime。 |
| `pkg/execution/recorder_runtime_api_test.go` | `SPLIT_JS_CONTRACT` | 是：execution `Run`、可信 capture capability、私有 Recorder backend/window/display seams 与 teardown | 真实 Goja execution 的 start/status/并发 stop/build 链及最终资源释放 | 内存输入 backend、固定窗口/显示器、临时录制目录 | 防止生产 JS 注入事件的同时验证 session handle、幂等 stop 和 execution cleanup | Go 仅提供 JS 无法安全公开的宿主 seam；公共调用断言保留在正式 `tests/runtime-api/seams/recorder-session.js` 中。 |
| `automation/accessibility_backend_darwin_test.go` | `SPLIT_JS_CONTRACT` | 是：Darwin `requireDarwinSetValueNotDisabled` 与原生 inspection 状态 | 缺失 `AXEnabled`、显式 enabled 和显式 disabled 三种 setValue 前置状态 | Darwin/cgo build tag；纯内存属性，不操作真实桌面 | 防止标准可写 `AXTextArea` 因省略 `AXEnabled` 被误拒绝，同时保持显式 disabled fail-closed | Go 只验证平台私有门禁；公开 `Accessibility.perform(..., { action: "setValue" })` 仍由 `tests/runtime-api/accessibility.js`、`tests/runtime-api/accessibility-native-macos.js` 与独立 Recorder 重放验证。 |
| `automation/keyboard_test.go` | `SPLIT_JS_CONTRACT` | 是：`normalizeKeyName` 到 robotgo modifier flag 的私有适配 | 公开 `Meta` 名称映射到 backend 接受的 `cmd` flag | 纯内存字符串映射，无真实键盘输入 | 防止生成脚本中的 `Meta+A` 被降级成普通 `a` 输入 | Go 只锁定 backend adapter seam；公开 keyboard 行为由 `tests/runtime-api/unit/keyboard.test.js`、live 输入 gate 与 Recorder 显式重放验证。 |
| `pkg/customui/notification_test.go` | `KEEP_PACKAGE` | 是：notification normalizer、session quota、capture visibility 与 ownership seam | 默认值、非法补丁、九宫格、跟随降级、终态与资源边界 | 内存 driver 与固定窗口几何 | 防止跨 execution 引用、迟到更新复活和截图污染 | 公开契约由 JS 用例负责；此文件只验证内部 seam。 |
| `pkg/http/inspector_test.go` | `KEEP_PACKAGE` | 是：Inspector service、API-only mux、配对/session token、TTL、并发、迟到提交与 artifact helper | loopback、精确 Host/Origin/Fetch Metadata、无页面/静态资源的 route isolation、真 runner 形状投影、review/import/handoff、撤销、shutdown 与端口释放 | `httptest`、伪造真实 `RemoteAddr`、内存 fake runner、`t.TempDir()` | 防止页面资源重新耦合、通用 execution/value/perform 路由旁路、跨 session、凭据重放、导入资格伪造、敏感树导出和迟到响应串窗 | HTTP/native controller 私有 seam；网页模型由 `tests/accessibility-workbench/model.test.js` 验证，真实 listener/AX 链由 `tests/accessibility-workbench/http-live.js` 验证。 |
| `pkg/http/inspector_control_test.go` | `KEEP_PACKAGE` | 是：按需 controller 的 generation、timer、私有 handler/CORS 配置和真实 listener 生命周期 | 普通服务默认无控制能力、loopback native 启动、独立静态页自连接的预检／精确 Origin／随机 API 端口、Host/远程 Origin/Fetch Metadata/forwarded 拒绝、frontend Origin 匹配、重复启动保护、配对 TTL、授权撤销和父服务 shutdown | `httptest`、真实 loopback 临时端口、短时 timer、`t.TempDir()` | 防止普通 App 启动时预开 Workbench、OpenDesk 抢占外部静态端口、远程或冒用 Origin 启用／读取 API、重复入口中断审阅，以及撤销后遗留 listener | 本文件锁定 host/controller/CORS 私有 seam；真实跨进程控制及独立静态服务合同由 `tests/accessibility-workbench/control-launch.js` 验证，原生取数仍由独立 HTTP/AX gate 负责。 |
| `pkg/inspector/runtime_test.go` | `KEEP_PACKAGE` | 是：固定内置程序、private result transport、validation status 与 window policy input helper | source-controlled runner 的只读能力摘要、无源码拼接/perform 和错误状态映射 | 当前平台 capability 初始化，不扫描窗口或读取树 | 防止内部程序扩展成任意脚本/动作入口，或遗漏 owner-level read-only/value policy | 只验证内部 runner 边界；真实目标 snapshot/validate 仍须当前 OpenDesk build 与平台 fixture。 |
| `pkg/entitlementservice/service_test.go` | `KEEP_PACKAGE` | 是：MemoryRegistry 的私有原子 device-slot/sequence state 与 Handler 状态映射 seam | TLS-only activate/refresh/deactivate、认证、设备上限、签名 cache sequence、revoke 与并发最后槽位原子性 | `httptest.NewTLSServer`、内存身份/registry、标准库密码学与 goroutine | 防止未授权签发、device limit 竞态、sequence replay 或明文 HTTP 降级 | service registry/issuer 组合白盒；只使用本地 TLS test server，不联系生产授权服务。 |
| `pkg/entitlement/client_test.go` | `KEEP_PACKAGE` | 是：HTTPClient 私有 request builder/transport guard、bounded response reader 与 status sanitizer seam | TLS 1.2 下限、系统/私有 CA、Bearer 请求、禁止 redirect/credential forwarding、响应上限/content type、状态映射与 activation id 安全 | 本地 `httptest` TLS/plain servers、内存请求 | 防止关闭证书校验、明文传输、token 转发、响应放大、服务端错误正文泄漏或路径注入 | authenticated transport 安全白盒；不依赖生产 endpoint。 |
| `pkg/licensing/online_entitlement_test.go` | `KEEP_PACKAGE` | 是：复用同包私有 device/license fixture、`memoryIssuerKeys` 与受控 secure replay store seam | signed online cache 严格解析/签名/nonce、offline grace、device binding、anti-replay watermark、revocation 与 authoritative cache 删除 | 标准库密码学、纯内存 secure store/repository | 防止 cache tamper、rollback/sequence reuse、revoked resurrection、错误 offline fallback 或 secure state 失败后放行 | Online License 内部密码学与 replay composition 白盒；HTTP transport 与 service lifecycle 由 entitlement/service 测试分层覆盖。 |
| `pkg/scriptpackage/scriptpackage_test.go` | `KEEP_PACKAGE` | 是：package-private `signatureDomain` 与原始 ZIP mutation fixture | `.odpkg` AES-GCM、Ed25519 domain、随机 key/nonce、raw manifest AAD、资源上限和篡改拒绝 | 标准库 crypto/rand、内存 ZIP；总大小 case 分配有界测试 buffer | 防止固定 nonce、错误签名消息、未知 entry、traversal、oversize 或 tamper 被接受 | 密码学与容器 parser 的同包安全白盒；不创建第二套 Runtime 或 production key。 |
| `pkg/scriptpackage/strict_json_test.go` | `KEEP_PACKAGE` | 是：package-private `rejectDuplicateObjectKeys` scanner 与共享 raw-package fixture | nested duplicate key、必填字段和严格 SemVer manifest 语法 | 内存 JSON/ZIP | 防止 duplicate shadowing、缺失 `license.required` 或模糊 runtime version 进入可信 manifest | strict decoder 私有门禁需同包验证，公开 package reader 结果同时覆盖。 |
| `pkg/scriptloader/scriptloader_test.go` | `KEEP_PACKAGE` | 是：同包 fake providers、delegate loader 与 caller-owned buffer 清零观测 | `.js` / `.odpkg` mode 绑定、验签授权解密顺序、UTF-8 payload、production fail-closed 和 key ownership | 临时 package 文件、标准库随机 key | 防止 mislabeled `.odpkg` 进入 plain snapshot、provider error key 残留或授权前解密 | loader composition/security seam 白盒；fake provider 只在测试内注入。 |
| `internal/protectedcli/protected_test.go` | `KEEP_PACKAGE` | 是：internal composition、package-private plaintext clearing 与 injected execute counter | protected Request 构造、package digest、snapshot 禁止、零执行门禁和 disclosure sentinel | 临时 artifacts、真实现有 Goja execution、测试 provider | 防止验签/License/key/decrypt 失败后启动 JS，或 sentinel/plaintext hash 落盘 | Host composition 安全白盒；成功链路仍调用唯一 `pkg/execution.Run()`。 |
| `internal/packagecli/packagecli_test.go` | `KEEP_PACKAGE` | 是：unexported `writeSecretFile`、publisher command parser 与 secret-file helper | protect/inspect/verify、0600 exclusive key output 和非敏感 JSON 输出 | 临时 Ed25519 文件与 `.odpkg` | 防止自动 DEK 无输出、secret file 覆盖或 inspect/verify 泄露 source/key | publisher-side CLI 私有装配测试，不把测试 key 注册为 Runtime default。 |
| `pkg/deviceidentity/identity_test.go` | `KEEP_PACKAGE` | 是：同包 secure-store fake、私有 device ID derivation 与持久化 item seam | 首次随机生成、重复读取稳定、损坏 secure-store fail closed、严格 public identity parsing | 纯内存 secure-store fake、标准库 P-256 与随机源 | 防止设备私钥被静默替换、损坏 key 被重生掩盖或伪造 DeviceID 被接受 | 设备身份安全白盒；public CLI 只导出公钥身份，不暴露私钥。 |
| `pkg/licensing/offline_license_test.go` | `KEEP_PACKAGE` | 是：license signature domain、strict nested JSON scanner、verified claims capability 与摘要化安装路径 seam | P-256/HKDF/AES-GCM envelope、Ed25519 License、时间/设备/包绑定、pin 冲突和 bounded file discovery | 标准库密码学、内存设备 store、`t.TempDir()` | 防止 wrong-device unwrap、metadata/tamper、duplicate JSON、伪造 Entitlement 或 key pin 替换绕过授权 | P1 license/crypto composition 白盒；不实现在线 entitlement 或 P3 key lifecycle。 |
| `internal/licensecli/licensecli_test.go` | `KEEP_PACKAGE` | 是：unexported command parser、bounded file reader、safe result projector 与 dependency injection seam | device/issue/inspect/verify/install 命令闭环及错误 JSON | 临时 package/key/license 文件、内存设备 identity | 防止 CLI 输出 DEK/private material、错误 key 发证、wrong-device 或篡改 License 被安装 | publisher/customer CLI 装配测试；production Keychain live 与 Runtime binary smoke 独立验收。 |
| `internal/protectedcli/device_license_test.go` | `KEEP_PACKAGE` | 是：同包 protected execution composition、已有 private test providers 与 injected execute counter | device-bound 授权进入唯一 Goja、no-license/wrong-device/expired/tamper 零执行和 disclosure sentinel | 临时 artifacts、真实现有 Goja、内存 secure-store/provider | 防止 P1 provider 绕过 P0 artifact policy、授权失败后启动 JavaScript 或 plaintext marker 落盘 | Host 安全集成白盒；普通 `.js` 与用户 CLI 另由既有 direct/AI regression 覆盖。 |
| `internal/licensecli/online_test.go` | `KEEP_PACKAGE` | 是：online command parser、可注入 client/replay guard、安装顺序与安全结果投影私有 seam | activate/status/refresh/deactivate 全链路、CA file 转交、nonce/sequence、cache 删除、commit 故障、wrong device/package/DEK 与秘密脱敏 | 临时 package/pin/cache/CA、内存 device/secure store、fake authenticated client | 防止未验证响应安装、部分提交后 fallback、旧 sequence 更新或 token/DEK/envelope 出现在 CLI JSON | 客户端在线授权装配白盒；真实 TLS transport 与服务 policy 分别由 `pkg/entitlement/client_test.go`、`pkg/entitlementservice/service_test.go` 覆盖。 |
| `internal/protectedcli/online_entitlement_test.go` | `KEEP_PACKAGE` | 是：同包 protected loader composition、P1 fixture 与 injected execution counter | active online entitlement 进入唯一 Goja；revoked、grace expiry、signature tamper、replay、wrong device 均在执行前失败 | 临时 `.odpkg`/artifact、内存 device/repository、真实现有 Goja | 防止 Online License 绕过 P0/P1 loader、失败后启动 JavaScript 或另建执行路径 | Host 安全集成白盒；复用 `DeviceBoundContentKeyProvider`，plain/disclosure 回归由既有 protected/plain gates 继续覆盖。 |
