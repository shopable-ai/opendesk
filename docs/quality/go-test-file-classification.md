...
| `automation/window_manager_windows_test.go` | `KEEP_PACKAGE` | 是：Windows 私有 observation/expected-state、Win32 error 分类和 PID normalizer helper | Window Manager state readback predicates、Win32 errno 映射与无损 PID 输入转换 | Windows build tag、纯内存状态与 `syscall.Errno`；不操作真实桌面 | 防止 hidden/maximized/stale 状态误判成功、权限/超时/失效句柄误分类和 PID 有损转换 | 平台 adapter 白盒；真实窗口仍由 `tests/windows/window-manager/run-live.ps1` 验证，不把 hosted fixture 冒充 interactive desktop。 |
| `cmd/opendesk/protected_recipe_route_test.go` | `KEEP_PACKAGE` | 是：main package 未导出的 `isAIProtectedRecipeInvocation` 与 `directProtectedPackagePath` | 普通 `.js` 与受保护 `.odpkg` 的 direct / `ai run` 路由边界 | 固定 argv vectors，无文件或网络 | 防止普通 JavaScript 被 protected route 截获，或 `.odpkg` 绕过保护链 | CLI composition 私有路由 seam；不创建第二套 execution。 |
| `pkg/appshell/manifest_test.go` | `KEEP_PACKAGE` | 是：同包 `validManifestJSON` fixture 被 `shell_test.go` 直接复用 | strict manifest decode、defaults、safe path、action namespace、menu 与 reopen policy | 纯内存 JSON 和 package-local fixture | 防止 unknown fields、多 JSON、路径逃逸、reserved action 与 hide 无 reopen path 被接受 | Parser 本身 exported，但同一受审 fixture 服务 shell 白盒；保留同包避免复制产生两套 App Shell contract。 |
| `tests/appshell/package_test.go` | `MOVE_GO_BLACKBOX` | 否：仅调用 exported `ResolvePackage`、`PackageIdentity` 与 `ManifestFileName` | canonical package root、required resources、symlink escape 与 package identity | `t.TempDir()` 文件树；非 Windows symlink fixture | 防止缺失资源或 symlink escape 被接受，并锁定 alias/manifest edit 不改变 package identity | 已从 `pkg/appshell/package_test.go` 迁至外部 package；filesystem package contract 不依赖实现私有符号。 |
| `pkg/appshell/shell_test.go` | `KEEP_PACKAGE` | 是：`defaultPendingActionCapacity` 私有常量、同包 `validManifestJSON` fixture 与 fake native | quit/teardown 状态、pre-bind queue、并发 dispatch、bounded queue、menu update 与 activation | 纯内存 fake native、goroutine | 防止 teardown 重复执行、queue overflow 静默、shutdown 后继续 dispatch 或 single-instance activation 脱离同一 host | App Shell 状态机白盒，公开脚本不应暴露这些 native lifecycle 内部状态。 |
