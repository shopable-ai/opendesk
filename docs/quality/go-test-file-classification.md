---
title: Go 测试逐文件分类清单
description: 2026-09-02 OpenDesk 测试架构审查中 145 个迁移前 *_test.go 资产的逐文件结论。
order: 21
---
...
- **继续与源码同包的 114 个文件**不是按路径批量保留。表中逐项区分了直接访问未导出实现的
  白盒测试、使用 fake/clock/EventLoop 的生命周期测试，以及虽只调用 exported Go API、但验证
  的仍是 Go package/CLI/MCP/持久化协议而不是 JavaScript Runtime 契约的测试。
...
| `pkg/container/container_test.go` | 虽主要调用 exported API，但测试的是 Go container ownership 和禁止 Runtime borrowing，不是 JavaScript surface。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| `.archive/batch1-sync-blockers-20260608-191116/pkg/container/container_test.go` | `ARCHIVE_ONLY` | 历史：旧副本曾访问 container 生命周期 | 旧同步批次的 runtime 借还与并发 | 历史 fake、clock | 只可解释旧实现预期，不证明当前包 | 保持 `.archive/` 隔离；当前证据是 `pkg/container/container_test.go`。 |
...
| `.archive/batch1-sync-blockers-20260608-191116/pkg/runtime/pool_test.go` | `ARCHIVE_ONLY` | 历史：旧 runtime pool 实现已退役 | Goja pool 借还、关闭、benchmark | goroutine、clock | 可供历史性能对照，不能证明现行 gate | 只作考古资料；当前 `pkg/runtime/pool_test.go` 测 execution gate。 |
| `.archive/batch1-sync-blockers-20260608-191116/pkg/semanticexec/mock_runtime_test.go` | `ARCHIVE_ONLY` | 历史：旧副本使用同包 scenario helper | semantic scenario 状态流 | 固定 fixture | 旧 happy/block/partial 预期 | 当前同名包测试是事实源，归档副本不运行。 |
| `.archive/batch1-sync-blockers-20260608-191116/pkg/semanticexec/status_test.go` | `ARCHIVE_ONLY` | 历史：旧副本验证状态派生 | terminal status 与 human gate | 无 | 旧状态优先级参考 | 归档隔离；不重复计入 145 之外的当前质量。 |
| `.archive/batch1-sync-blockers-20260608-191116/pkg/semanticexec/verify_test.go` | `ARCHIVE_ONLY` | 历史：旧副本验证 verifier helper | false-success 与 degraded/partial | 固定 fixture | 旧 verifier 规则参考 | 归档隔离；当前测试在 `pkg/semanticexec/verify_test.go`。 |
...
| `automation/browser_lifecycle_test.go` | `KEEP_PACKAGE` | 否：只调用 exported Browser/Context 方法 | 默认 context 所有权、closed guard、幂等 close | 无，纯内存 | 防止关闭后复活或 page 跨 context 泄漏 | 仍是 Go 对象生命周期 package contract，不是用户 JS 参数/返回契约。 |
...
| `automation/layout_test_helpers_test.go` | `KEEP_PACKAGE` | 是：共享 `layoutSeparator`、region parse 与断言 helper | 仅 test build 的 package-private fixture/helper | 无 | 自身无 Test，但被 layout tests 调用并 fail-fast | helper 不能变成独立工具；保留 `_test.go` 且不单独计 pass。 |
...
| `automation/vision_js_integration_test.go` | `SPLIT_JS_CONTRACT` | 否：经已注入 `Vision.runOCR` 调用，但使用 fake provider | typed array 输入穿过真实 Goja binding 的 integration seam | Goja、fake OCR provider | 防止 JS TypedArray 到 native bytes 的转换断裂 | 保留 bridge integration；公共 Vision 由 `tests/runtime-api/unit/vision.test.js`。 |
| `automation/vision_layout_test.go` | `KEEP_PACKAGE` | 否：调用 exported `Vision.AnalyzeLayout`，检查 native result shape | generic hints 到 layout algorithm 的 Go integration | 合成 PNG、`t.TempDir()` | 防止 hints 解析和 separator/region shape 丢失 | 这是 Go native 算法集成；JS shape 另有 `tests/runtime-api/unit/vision-layout.test.js`。 |
...
| `pkg/benchmark/semantic_smoke_test.go` | `KEEP_PACKAGE` | 否：调用 exported semantic suite | 语义 smoke 覆盖矩阵与报告稳定性 | 固定 fixture | 防止缺少 outcome 类别或 false-success case 仍通过 | package 级质量算法；应由 Go gate 执行，不是独立生成工具。 |
| `pkg/container/container_test.go` | `KEEP_PACKAGE` | 否：主要调用 exported Container API | container owner、close 与禁止 runtime borrowing | fake config/service | 防止重新暴露 raw Runtime 或 close 后资源仍可用 | Go container ownership contract，与 JS 用户 API 无关。 |
| `pkg/customui/icons_test.go` | `KEEP_PACKAGE` | 否：检查 exported reviewed registry | SF Symbol allowlist 的完整性与 presentation metadata | 仓库生成表 | 防止重复、无 label 或未审核图标进入 toolbar | native UI 资源 package invariant。 |
...
| `pkg/customui/queue_test.go` | `KEEP_PACKAGE` | 否：通过 EventQueue API 检查内部队列语义 | UI event 顺序、允许的 coalescing、overflow/close | 纯内存 | 防止高频事件乱序或 overflow 静默丢失 | Go 并发数据结构测试，不是 JS listener contract。 |
| `pkg/customui/session_concurrency_test.go` | `KEEP_PACKAGE` | 否：使用 fake Driver 操作 Session/Window state | create/close 竞态、retryable host failure、terminal close | goroutine、fake driver | 防止 duplicate ID race、close 后复活和 host failure 半关闭 | native session concurrency seam。 |
...
| `pkg/customui/toolbar/model_test.go` | `KEEP_PACKAGE` | 否：调用 toolbar model API | 生成 icon registry、orientation 与 wrapping policy | 无 | 防止 registry 非有序完整、vertical 超限、horizontal 计算溢出 | toolbar 纯 Go policy，留模型包。 |
...
| `pkg/desktopvision/annotate_test.go` | `KEEP_PACKAGE` | 否：调用 exported annotation API | bbox/center 绘制和 PNG artifact | 内存 image、`t.TempDir()` | 防止标注坐标和文件编码回归 | desktopvision 算法 package 测试。 |
| `pkg/desktopvision/coordinates_test.go` | `KEEP_PACKAGE` | 否：调用 exported coordinate resolvers | normalized/image/window/screen 坐标转换 | 固定 geometry | 防止零尺寸、越界 bbox 和 scale 转换错误 | 纯 Go geometry contract。 |
| `pkg/desktopvision/gates_test.go` | `KEEP_PACKAGE` | 否：调用 exported action gate | 唯一目标、风险、置信度与 stale fail-closed | 固定 target records | 防止 ambiguous/stale target 被执行 | 安全决策 package contract，不是 Runtime surface。 |
| `pkg/desktopvision/provider_test.go` | `KEEP_PACKAGE` | 是：`perceptionSchema`；其余经 fake provider | provider JSON schema、provenance、timeout 与 coordinate resolve | fake model、clock、`t.TempDir()` | 防止 unknown fields、截图 SHA 不符或超时仍接受感知 | provider/security seam 保留包内。 |
| `pkg/desktopvision/script_test.go` | `KEEP_PACKAGE` | 是：同包 window dimensions/helper | deterministic replay script generation | 固定 Flow IR | 防止缺目标 action、遗漏 post-verification 或 dry-run 丢失 | 编译器 package 测试；生成字符串有精确断言，不是人工工具。 |
| `pkg/desktopvision/trace_test.go` | `KEEP_PACKAGE` | 否：调用 TraceRecorder/normalize API | NDJSON trace 与坐标 copy | `t.TempDir()` | 防止 trace 非结构化或 target pointer alias | artifact writer package invariant。 |
| `pkg/desktopvision/types_test.go` | `KEEP_PACKAGE` | 否：marshal exported domain types | perception JSON schema 与 Risk 排序 | 无 | 防止协议字段和安全等级顺序漂移 | Go domain schema contract。 |
| `pkg/execution/desktop_events_test.go` | `KEEP_PACKAGE` | 否：用 Runtime options/fake backend 驱动完整 execution | Events 让 execution 存活、回调和结束 cleanup | fake desktop backend、goroutine、clock | 防止 runner 过早退出或 teardown 后 subscription 残留 | execution 生命周期 integration；公共 Event shape 仍由 JS unit。 |
| `pkg/execution/global_shortcut_test.go` | `KEEP_PACKAGE` | 否：用 fake shortcut backend 运行真实 runner | shortcut callback、keepalive、failure cleanup | fake backend、goroutine、clock | 防止 callback failure 后注册残留或 execution 不结束 | runner ownership integration，不替代 JS contract。 |
| `pkg/execution/manager_test.go` | `KEEP_PACKAGE` | 否：调用 Manager API | 并发 execution registry、CancelAll/WaitAll、shutdown admission | goroutine | 防止 shutdown 后仍接收任务或 WaitAll 提前返回 | execution service 并发 contract。 |
...
| `pkg/execution/runner_lifecycle_test.go` | `KEEP_PACKAGE` | 否：经 RunJavaScript 驱动 runner lifecycle | async HTTP、Abort、unhandled rejection、deadline 与 goroutine drain | loopback HTTP、clock、goroutine、临时 artifacts | 防止 Promise/cancel 后 worker 或 goroutine 累积 | execution 层集成 seam；用户 surface 另由 JS catalog。 |
| `pkg/execution/runner_test.go` | `KEEP_PACKAGE` | 否：经 injected fake driver/backend 运行真实 execution | Custom UI、Dialog、stack、Execution、Native Extension opt-in 与 timeout cleanup | fake UI、loopback、clock、`t.TempDir()` | 防止 capability 越权、unawaited resource 泄漏和 busy loop 不可中断 | runner orchestration contract，不宜搬为 Goja public unit。 |
| `pkg/execution/runtime_ownership_test.go` | `KEEP_PACKAGE` | 否：静态读取 owner source files | EventLoop ownership 和禁止跨 goroutine 直接触碰 Goja 的静态约束 | 仓库源码文件 | 防止关键 `RunOnLoop`/`Interrupt` 约束被移除 | 实现静态 invariant；不是运行用户 API。 |
| `pkg/http/handler_test.go` | `KEEP_PACKAGE` | 否：通过 Server API 与 fake execution 断言 transport | HTTP routes、capability、cancel/shutdown、并发和 status | `httptest` loopback、fake UI、clock | 防止远程 UI 越权、shutdown 留资源和 stack 默认漂移 | HTTP transport/server package contract。 |
...
| `pkg/mcpserver/recorder_test.go` | `KEEP_PACKAGE` | 否：通过 MCP Server 与 Recorder API | action trace、distill/compile 与 wrong-window stop | `t.TempDir()`、fake runtime | 防止记录越过 guard 或 deterministic compile 含 AI call | MCP Recorder integration，不是 Runtime JS API。 |
...
| `pkg/nativeextension/process_unix_test.go` | `KEEP_PACKAGE` | 否：调用 Host 但观察 process group | timeout/cancel 杀整个 Unix extension process group | helper shell process、clock、`t.TempDir()` | 防止只杀父进程留下 child | Unix lifecycle integration，必须在包 gate。 |
...
| `pkg/operator/semantic_maintenance_test.go` | `KEEP_PACKAGE` | 否：调用 exported semantic fixture audit | operator semantic fixtures 的维护约束 | 仓库 fixture | 防止 fixture 缺关键 outcome 或 schema 漂移 | package maintenance gate，有断言而非报告生成器。 |
| `pkg/recorder/recorder_test.go` | `KEEP_PACKAGE` | 否：主要通过 Store/Distill/Compile API，使用包内 fixture | session 隔离、tail recovery、redaction、Flow IR、deterministic replay | `t.TempDir()`、goroutine | 防止 secret 泄漏、damaged tail 破坏恢复和 ambiguous target 被执行 | Recorder domain package contract；MCP wire 另测。 |
...
| `pkg/runtime/pool_test.go` | `KEEP_PACKAGE` | 否：调用 ExecutionGate API | 并发容量、release 与 close admission | context/clock | 防止容量泄漏和 close 后新 acquire | 当前不是旧 Goja pool；是 service concurrency contract。 |
| `pkg/runtimeconfig/config_test.go` | `KEEP_PACKAGE` | 否：通过 Load/ResolveUI API | strict config、UI 优先级、legacy fallback | `t.TempDir()` config 文件 | 防止未知字段、显式缺失被静默 fallback | 配置 package contract。 |
...
| `pkg/scheduler/schedule_test.go` | `KEEP_PACKAGE` | 否：调用 schedule parser | every/cron next-run 与非法表达式 | 固定 clock | 防止接受非标准 cron 或负 interval | 纯 Go 时间算法 package test。 |
| `pkg/scheduler/service_test.go` | `KEEP_PACKAGE` | 否：以 fake executor 驱动 Service | file/inline CRUD、restart recovery、misfire、fixed delay、serialization | temp DB/files、fake clock、goroutine | 防止 restart 丢任务、misfire 重放多次和并发 RunNow | Scheduler service state machine。 |
...
| `pkg/semanticexec/mock_runtime_test.go` | `KEEP_PACKAGE` | 否：通过 exported scenario runner/fakes | happy、permission blocked、partial、false-success、budget exhaustion | 固定内存 fixture | 防止 outcome 覆盖不闭合 | domain state-machine package test。 |
| `pkg/semanticexec/status_test.go` | `KEEP_PACKAGE` | 否：调用 exported status helpers | status precedence、terminal 和 human gate | 无 | 防止 blocked/false-success 优先级回归 | 纯 Go domain rule。 |
| `pkg/semanticexec/verify_test.go` | `KEEP_PACKAGE` | 否：调用 exported verifier helpers | business verifier、degraded/partial 与 false-success | 固定 fixture | 防止 action success 掩盖业务验证失败 | 纯 Go verification policy。 |
