...
| `pkg/customui/toolbar_image_windows_test.go` | `KEEP_PACKAGE` | 是：Windows build-tag 下的 `platformToolbarLocalPath`、私有 image loader 与测试图像 helper | drive/UNC/extended path 规范化、Unicode/空格路径、非法 namespace/ADS 与 junction escape 拒绝 | Windows 路径语义、临时 PNG、`cmd /c mklink /J` | 防止 drive-relative、device namespace、查询后缀或 junction 越出受信 UI asset root | Windows Custom UI 路径安全白盒，必须留在同包；cross-compile 不替代 Windows live junction 验证。 |
