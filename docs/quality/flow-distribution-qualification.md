...
| E3 | `go test ./pkg/customui ./pkg/appshell ./pkg/flowinstall ./pkg/flowpackage ./internal/flowcli ./pkg/execution ./pkg/appdata -count=1`；`node --test tests/custom-ui/script-runner-simple.test.js` | PASS；Node 29/29。覆盖路径边界、service/install、Catalog、稳定 install ID、明确 run、安装不自动执行和刷新。 |
