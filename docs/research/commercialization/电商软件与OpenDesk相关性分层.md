...
>
> 本文解决一个问题：**不是所有“电商软件”都与 OpenDesk 同等相关。** 研究对象必须先判断它与 OpenDesk 的技术、产品和业务距离，再决定研究深度。不能因为某个电商 SaaS 很赚钱，就把它直接写成 OpenDesk 的竞品或近期开发方向。
...
这个软件是否直接替代 OpenDesk？
↓
这个软件是否是 OpenDesk 可以直接自动化的目标应用？
↓
它是否已经解决了 OpenDesk 想解决的同一个业务 Workflow？
↓
它是否只是给 OpenDesk 提供上游数据 / 决策信号？
↓
它是否仅仅提供商业模式参考？
```
...
## 2. OpenDesk 关联等级：C0—C4

### C0：直接技术竞品 / 替代方案

定义：

- 能直接完成 OpenDesk 核心自动化能力；
- 用户可以在“使用 OpenDesk”与“使用它”之间直接二选一；
- 会影响 OpenDesk 是否还需要自己建设某个底层能力。
...
- 用户群；
- 商业模式；
- OpenDesk 的 Build / Buy / Integrate / Avoid 决策。
...
### C1：OpenDesk 直接自动化目标 / 第一商业化场景

定义：

- OpenDesk 可以直接操作其 Desktop / Web UI；
...
- 商家可能因为已经购买这些软件，而不再需要 OpenDesk 的对应 Workflow。
...
- 与 OpenDesk 的核心“执行”能力距离较远；
- 更可能成为 OpenDesk 的输入信号、Trigger 或外部数据源，而不是直接竞品。
...
- 哪些洞察可以触发具体执行；
- 收费方式和高价值功能；
- OpenDesk 是否能接在它后面形成执行闭环。

**研究深度：中。**

> 不应因为 Helium 10、Jungle Scout、飞瓜很赚钱，就推导 OpenDesk 自己建设同类大型数据平台。
...
- 与 OpenDesk 当前技术和产品路径没有直接替代关系；
- 但其定价、套餐、Marketplace、按使用量、按结果收费、企业版等商业模式值得借鉴；
- 主要用于回答“软件怎样赚钱”，而不是“OpenDesk 应该开发它的功能”。
...
单一“OpenDesk 相关等级”仍然不够。

例如：

- Gorgias 在底层技术上不是 OpenDesk 的直接竞品；
- 但“按成功解决结果收费”对 OpenDesk 的商业模式参考价值很高。

因此增加独立商业参考等级：

| 等级 | 定义 | 处理方式 |
|---|---|---|
| B3 | 商业模式可直接启发 OpenDesk | 深入记录价格、收费单位、套餐、价值指标、客户 |
...
| 软件 / 平台 | OpenDesk 关联等级 | 商业参考 | 当前角色 | 研究优先级 |
|---|---|---|---|---|
...
一个候选功能若想进入 OpenDesk 电商 MVP，至少需要满足：
...
市场：国内 / 海外 / 跨境
类别：
OpenDesk 关联等级：C0 / C1 / C2 / C3 / C4
商业参考等级：B0 / B1 / B2 / B3
OpenDesk 角色：竞品 / 自动化目标 / 替代方案 / 数据源 / 商业参考
研究优先级：P0 / P1 / P2 / P3
用户为什么付钱：
收费方式：
与 OpenDesk 的直接关系：
OpenDesk 是否应 Build / Integrate / Automate / Avoid：
下一步需要验证什么：
```

没有填写“与 OpenDesk 的直接关系”的软件，不应继续投入大量研究时间。
