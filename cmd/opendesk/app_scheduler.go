...
func startAppScheduler(ctx context.Context, config *Config, packageID, appRoot string, environment map[string]string) (*appSchedulerRuntime, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("listen for App Scheduler bridge: %w", err)
	}
	...
	go func() {
		...
		if serveErr != nil && !errors.Is(serveErr, http.ErrServerClosed) {
			terminalPrintf(os.Stderr, "[FRAMEWORK] [WARN] App Scheduler bridge stopped: %v\n", serveErr)
		}
	}()
	cleanupStore = false
	return runtime, nil
...
func (r *appSchedulerRuntime) close(ctx context.Context) error {
	r.closeOnce.Do(func() {
		...
		var serverErr error
		if r.server != nil { serverErr = r.server.Shutdown(ctx) }
		var schedulerErr error
		if r.service != nil { schedulerErr = r.service.Close(ctx) }
		var listenerErr error
		if r.listener != nil {
			listenerErr = r.listener.Close()
			if errors.Is(listenerErr, net.ErrClosed) { listenerErr = nil }
		}
		var storeErr error
		if r.store != nil { storeErr = r.store.Close() }
		r.closeErr = errors.Join(serverErr, schedulerErr, listenerErr, storeErr)
	})
	return r.closeErr
}
...
func resolveAppSchedulerScriptRoot(packageID, appRoot string, environment map[string]string) (string, error) {
	...
	if configured != "" { return normalizeAppSchedulerRoot(configured, appRoot) }

	appDataRoot := strings.TrimSpace(environment["OPENDESK_APP_DATA_DIR"])
	if appDataRoot == "" {
		home := strings.TrimSpace(environment["HOME"])
		if home == "" { home = strings.TrimSpace(environment["USERPROFILE"]) }
		if home == "" {
	...
	if !filepath.IsAbs(appDataRoot) { appDataRoot = filepath.Join(appRoot, appDataRoot) }
...
func normalizeAppSchedulerRoot(root, appRoot string) (string, error) {
	if !filepath.IsAbs(root) { root = filepath.Join(appRoot, root) }
	absolute, err := filepath.Abs(root)
	if err != nil { return "", fmt.Errorf("resolve App Scheduler script root: %w", err) }
	return filepath.Clean(absolute), nil
}

func randomAppSchedulerToken() (string, error) {
	buffer := make([]byte, 32)
	if _, err := rand.Read(buffer); err != nil { return "", fmt.Errorf("create App Scheduler bridge token: %w", err) }
