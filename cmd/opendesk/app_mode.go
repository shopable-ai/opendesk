...
func validateAppModeConfig(config *Config) error {
	...
	return validateAppModeConfigFull(config)
}

func validateAppModeConfigFull(config *Config) error {
...
func validateAppModeHelperConflict(args []string) error {
	if !appModeRequested(args) { return nil }
	notificationHelper := false
	for _, arg := range args { if automation.MacOSNotificationHelperRequested([]string{arg}) { notificationHelper = true; break } }
	if notificationHelper || automation.MacOSRegionSelectorHelperRequested(args) { return errors.New("-app is mutually exclusive with internal helper modes") }
	return nil
}
...
func executeAppMode(config *Config) error {
	...
	appPackage, err := appshell.LoadPackage(config.AppPath); if err != nil { return err }
	appPackage.Manifest = appshell.EnsureRecorderMenu(appPackage.Manifest)
	content, err := os.ReadFile(appPackage.EntryPath); if err != nil { return fmt.Errorf("read App Mode entry: %w", err) }
	environment, err := runtimeenv.Resolve(runtimeenv.Options{WorkingDirectory: appPackage.Root, File: config.EnvironmentFile, Inherited: os.Environ()}); if err != nil { return fmt.Errorf("resolve App Mode environment: %w", err) }
	nativeHost, err := appshell.NewNativeHost(appPackage); if err != nil { return err }
	shell, err := appshell.New(appPackage.Manifest, nativeHost); if err != nil { return err }
	sharedUIDriver := customui.NewProcessDriver(customui.ProcessDriverOptions{HostPath: config.CustomUIHostPath}); defer sharedUIDriver.Close()
	signalContext, stopSignals := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM); defer stopSignals()
	appContext, cancelApp := context.WithCancel(signalContext); defer cancelApp()
	shell.SetQuitHook(cancelApp)
	stopSignalHook := context.AfterFunc(signalContext, func(){ _ = shell.RequestQuit() }); defer stopSignalHook()
	lease, primary, err := appshell.AcquireSingleInstance(appContext, appPackage.Manifest, func() bool { return shell.Activate("second-instance") == nil }); if err != nil { return err }
	...
	appScheduler, err := startAppScheduler(appContext, config, appPackage, environment); if err != nil { return fmt.Errorf("start App Scheduler: %w", err) }
	...
	artifacts, err := pkgExecution.PrepareArtifacts(config.LogDir, executionID, ".js"); if err != nil { return errors.Join(err, shell.Teardown()) }
	...
	mainErr := shell.RunMain(appContext); if mainErr != nil { _ = shell.RequestQuit() }
	outcome := <-runResult
	var leaseErr error
...
func appModeRecorderCaptureAllowed(config *Config) bool {
	...
	defaultPackage := bundledAppModePath(); if defaultPackage == "" || strings.TrimSpace(config.AppPath)=="" { return false }
	requested, err := filepath.Abs(config.AppPath); if err != nil { return false }
