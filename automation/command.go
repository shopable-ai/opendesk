...
import (
	...
	"opendesk/automation/internal/commandresolver"
...
const (
	...
	commandEventChunkBytes  = 64 * 1024
)
...
func registerCommand(runtimeValue *goja.Runtime, opts InitJSOptions) *CommandRuntime {
	...
	// Agent adapters need the same platform PATH/PATHEXT and executable checks
	// as the process owner, but this is not a public Command API. The AI facade
	// captures this non-enumerable bridge while polyfills load and immediately
	// removes it before user JavaScript runs.
	resolver := runtimeValue.ToValue(func(call goja.FunctionCall) goja.Value {
		candidate, ok := call.Argument(0).Export().(string)
		if !ok || len(call.Arguments) != 1 {
			return runtimeValue.ToValue(map[string]interface{}{"path": "", "status": commandresolver.StatusInvalid})
		}
		resolved := commandresolver.Resolve(candidate, manager.environment)
		return runtimeValue.ToValue(map[string]interface{}{"path": resolved.Path, "status": resolved.Status})
	})
	_ = object.DefineDataProperty(
		"__resolveExecutable",
		resolver,
		goja.FLAG_FALSE,
		goja.FLAG_TRUE,
...
func (c *CommandRuntime) launch(spec commandSpec, waiter commandWaiter) (*commandProcess, error) {
	...
	cmd.Dir = spec.cwd
	cmd.Env = spec.env
	configureCommand(cmd, spec.hideWindow)
...
func (w *commandOutputWriter) Write(value []byte) (int, error) {
	...
	accepted, messages := w.process.appendOutput(value, w.stderr)
	w.process.emitOutput(w.stderr, messages)
	if !accepted {
		w.process.forceKill()
	}
...
func (p *commandProcess) appendOutput(value []byte, stderr bool) (bool, []string) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.overflow {
		return false, nil
	...
	if stderr {
		...
		return true, commandEventMessages(&p.stderrPending, value, false)
	} else {
		p.stdout = append(p.stdout, value...)
		return true, commandEventMessages(&p.stdoutPending, value, false)
	}
}

func (p *commandProcess) emitOutput(stderr bool, messages []string) {
	if p == nil || !p.spec.emitOutput || p.owner == nil || p.owner.eventSink == nil {
		return
	}
	stream, kind, level := "stdout", "command.stdout", "info"
	if stderr {
		stream, kind, level = "stderr", "command.stderr", "warn"
	}
	fields := map[string]any{"stream": stream, "command": p.spec.command}
	if p.cmd != nil && p.cmd.Process != nil {
		fields["pid"] = p.cmd.Process.Pid
	}
	for _, message := range messages {
		p.owner.eventSink.Emit("script", level, "runtime", kind, message, fields)
	}
}

func (p *commandProcess) flushOutputEvents() {
	p.mu.Lock()
	stdout := commandEventMessages(&p.stdoutPending, nil, true)
	stderr := commandEventMessages(&p.stderrPending, nil, true)
	p.mu.Unlock()
	p.emitOutput(false, stdout)
	p.emitOutput(true, stderr)
}

// commandEventMessages preserves split UTF-8 sequences and line boundaries for
// structured streaming while keeping a bounded partial-line buffer. The exact
// child bytes remain owned by stdout/stderr above for Command.run's result.
func commandEventMessages(pending *[]byte, value []byte, flush bool) []string {
	*pending = append(*pending, value...)
	...
	for len(buffer) > 0 {
		if index := bytes.IndexByte(buffer, '\n'); index >= 0 {
			...
			messages = append(messages, validCommandOutput(line))
			buffer = buffer[index+1:]
			continue
		}
		if flush {
			messages = append(messages, validCommandOutput(buffer))
			buffer = nil
			break
		}
		if len(buffer) < commandEventChunkBytes {
			break
		}
		cut := commandUTF8PrefixLen(buffer, commandEventChunkBytes)
		if cut == 0 {
			break
		}
		messages = append(messages, validCommandOutput(buffer[:cut]))
		buffer = buffer[cut:]
	}
	*pending = append((*pending)[:0], buffer...)
	return messages
}

func commandUTF8PrefixLen(value []byte, maximum int) int {
...
func parseCommandInvocation(call goja.FunctionCall, environment []string) (commandSpec, goja.Value, error) {
	spec := commandSpec{
		...
		maxOutputBytes: commandDefaultMaxOutput,
		emitOutput:     true,
	}
...
func parseCommandOptions(value goja.Value, spec *commandSpec) error {
	...
	allowed := map[string]bool{"cwd": true, "env": true, "envMode": true, "timeout": true, "maxOutputBytes": true, "hideWindow": true, "emitOutput": true, "input": true, "signal": true}
	...
	envMode := "inherit"
	if raw, exists := options["envMode"]; exists {
		mode, ok := raw.(string)
		if !ok || (mode != "inherit" && mode != "replace") {
			return commandOperationError(CommandInvalidArg, "envMode must be inherit or replace", nil)
		}
		envMode = mode
	}
	...
	if raw, exists := options["env"]; exists {
		...
		env, err := mergeCommandEnv(base, envObject)
		if err != nil {
			return commandOperationError(CommandInvalidArg, err.Error(), err)
		}
		spec.env = env
	} else if envMode == "replace" {
		spec.env = []string{}
	}
	...
	if raw, exists := options["hideWindow"]; exists {
		hideWindow, ok := raw.(bool)
		if !ok {
			return commandOperationError(CommandInvalidArg, "hideWindow must be a boolean", nil)
		}
		spec.hideWindow = hideWindow
	}
	if raw, exists := options["emitOutput"]; exists {
		emitOutput, ok := raw.(bool)
		if !ok {
			return commandOperationError(CommandInvalidArg, "emitOutput must be a boolean", nil)
		}
		spec.emitOutput = emitOutput
	}
