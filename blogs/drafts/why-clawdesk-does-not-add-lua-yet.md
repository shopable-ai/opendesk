...
很多自动化 Runtime 都支持多种脚本语言，所以当 Clawdesk 已经拥有 JavaScript Runtime 后，一个自然的问题是：
...
Clawdesk 更重要的扩展问题并不是“再多一种脚本语法”，而是：

> 用户在没有 Clawdesk Core 源码的情况下，怎样增加 Runtime 当前不存在的底层能力？
...
如果 Clawdesk 没有向 Lua 暴露对应 Host Function，那么 Lua 同样无法调用它。
...
Clawdesk 当前更需要把资源投入到真正扩大能力边界和可靠性的事情上，例如：
...
这不要求第三方得到 Clawdesk 核心源码，却能真正增加新的 Native 能力。

## 为什么 JavaScript 暂时足够承担脚本层

Clawdesk 已经拥有 JavaScript Runtime，并且当前 Polyfill / Runtime API / examples / types / tests 都围绕 JavaScript 建立。
...
Clawdesk 并不是永久拒绝 Lua。

以下任一条件开始真实出现，就值得重新评估：

1. 有明确用户持续要求 Lua。
2. 有大量现成 Lua 自动化资产需要迁移。
3. Clawdesk 进入 Lua 占主导的垂直生态。
4. Clawdesk 产品定位升级为多语言 Automation Runtime。
...
> **现在增加 Lua 主要扩大语言选择，却不会显著扩大 Clawdesk 的能力边界。**
