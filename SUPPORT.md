...
本页是 Clawdesk 的统一支持与定制入口。

如果只是想组合已有能力，请先阅读：

- `docs-user-api/custom-api.md`：只使用现有 JavaScript API 自助扩展。
- `docs-user-api/index.md`：当前公开 API 地图。
...
优先通过 HTTP 或 MCP 集成，而不是把所有依赖编译进 Clawdesk。
...
如果你只有二进制发行版、没有源码权限，或者希望得到正式支持的构建，请联系 Clawdesk 项目作者 / 维护者提出定制需求。
...
具体是否适合进入 Clawdesk 核心、做成外置服务或保持客户私有，由需求评估后决定。
...
请优先使用你获取 Clawdesk 时对应的**官方项目 / 产品联系渠道**联系项目作者或维护者，并附上上面的定制需求信息。
