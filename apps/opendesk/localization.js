(function installOpenDeskProductLocalization(global) {
  ...
  const entries = [
    ...
    ['product.mainTitle', 'OpenDesk — 自动化'], ['product.automation', '自动化'],
    ['product.noFlow', '暂无可运行的流程'], ['product.flowList', '流程列表'],
    ['product.customize', '定制'], ['product.help', '帮助'],
    // Flow Runner: Runnable Entry states, actions, and execution feedback.
    ['flowRunner.title', 'OpenDesk — 自动化'], ['flowRunner.directory', '流程目录'],
    ['flowRunner.directoryUnavailable', '流程目录不可用'], ['flowRunner.loadingEntries', '正在读取流程目录…'],
    ['flowRunner.loading', '正在加载流程…'], ['flowRunner.loadingHelp', '正在读取流程目录，请稍候。'],
    ['flowRunner.empty', '暂无可运行的流程'], ['flowRunner.emptyHelp', '将流程添加到流程目录后，可以在这里直接运行。'],
    ['flowRunner.openDirectory', '打开自动化目录'], ['flowRunner.refreshList', '刷新流程列表'],
    ['flowRunner.listLoadFailed', '流程列表加载失败'], ['flowRunner.rescan', '重新扫描自动化目录'],
    ['flowRunner.entry', '流程'], ['flowRunner.action', '操作'], ['flowRunner.order', '排序'],
    ['flowRunner.runSelected', '运行选中的流程'], ['flowRunner.stopRun', '停止'],
    ['flowRunner.restoreOrder', '恢复默认排序'], ['flowRunner.closeList', '关闭流程列表'],
    ['flowRunner.noEntries', '暂无可运行的流程。'], ['flowRunner.running', '正在运行'],
    ['flowRunner.completed', '运行完成'], ['flowRunner.runFailed', '运行失败'], ['flowRunner.stopped', '已停止'],
    ['flowRunner.rescanning', '正在重新扫描流程目录…'], ['flowRunner.rescanned', '已刷新'],
    ['flowRunner.unknownError', '未知错误'], ['flowRunner.permissionRequired', '所需系统权限'],
    ['flowRunner.startedCaption', 'OpenDesk 已开始执行，可随时点击停止。'],
    ['flowRunner.permissionsCaption', '请打开菜单“系统权限…”处理后重试。'],
    ['flowRunner.logSaved', '执行日志已保存。'], ['flowRunner.remainingStopped', '剩余流程不会继续执行。'],
    ...
    ['recorder.noGeneratedScript', '该录制还没有可运行的生成脚本。'],
