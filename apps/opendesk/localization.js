(function installOpenDeskProductLocalization(global) {
  ...
  const entries = [
    ...
    ['settings.title', '设置'], ['settings.description', '管理 OpenDesk 的产品偏好。'],
    ['settings.privacyData', '隐私与数据'], ['settings.privacyDescription', '控制 OpenDesk 是否可以发送基础使用数据。'],
    ['settings.improve', '帮助改进 OpenDesk'], ['settings.improveDescription', '开启后，OpenDesk 会发送应用启动、功能使用和自动化运行结果等基础使用信息，用于改进产品。'],
    ['settings.reading', '正在读取…'], ['settings.readingState', '正在读取设置状态…'],
    ['settings.privacySummary', '不会发送脚本或 Flow 源码、截图、剪贴板、OCR 内容、AI 对话、Prompt、用户输入、本地完整路径或原始错误文本。'],
    ['settings.privacyDetails', '查看隐私说明'], ['settings.enabled', '已开启'], ['settings.disabled', '已关闭'],
    ['settings.enabledDetail', '已开启。你的选择已保存；服务可用时会发送上述基础使用数据。'],
    ['settings.disabledDetail', '已关闭。OpenDesk 不会发送基础使用数据。'],
    ['settings.readFailed', '暂时无法读取此设置。请稍后重试。'], ['settings.updateFailed', '无法更新此设置。原有选择保持不变。'],
    ['settings.privacyTitle', '隐私说明'],
    ['settings.privacyP1', '启用“帮助改进 OpenDesk”后，只会发送受控的基础使用信息，用于了解产品功能是否被使用以及自动化运行是否正常。'],
    ['settings.privacyP2', 'OpenDesk 不发送脚本或 Flow 源码、截图、剪贴板、OCR 内容、AI 对话、Prompt、用户输入、本地完整路径、Secret、Token 或原始错误文本。'],
    ['settings.privacyP3', '启用后会生成随机安装标识，用于区分重复使用和会话；不会使用机器序列号、MAC、硬盘或 CPU 标识、License ID 或账号 ID。关闭后停止接受新的基础使用数据，并撤销本地统计标识。'],
    ['developer.analyticsTitle', '产品统计诊断'], ['developer.analyticsDescription', '仅用于开发与诊断；这里不是业务 Dashboard。'],
    ['developer.analyticsInitialization', '初始化状态'], ['developer.analyticsQueue', 'Queue 状态'],
    ['developer.analyticsLastSend', '最近发送结果'], ['developer.analyticsErrors', '丢弃 / 最近错误'],
    ['developer.analyticsRecentEvents', '最近事件'],
    ['developer.analyticsNotice', '“已进入 Provider 队列”只表示本地 Provider 接受了事件，不代表云端已经接收或写入 Dashboard。'],
    ['developer.analyticsNoEvents', '尚无经过隐私与 Schema 校验的诊断事件。'],
    ['developer.analyticsQueued', '已进入 Provider 队列（非云端确认）'],
    ['developer.analyticsDebugStored', '已写入本地 Debug Ring'],
    ['developer.analyticsProviderUnavailable', 'Provider 不可用'],
    ['developer.analyticsEnqueueFailed', 'Provider 入队失败'],
    ['developer.analyticsNoSend', '尚无发送结果'],
