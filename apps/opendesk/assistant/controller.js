(function installOpenDeskAssistantController(global) {
  ...
  function pageSlice(items, page, size) {
    ...
    const safePage = Math.max(0, Math.min(Number.isInteger(page) ? page : 0, maxPage));
    return {page: safePage, maxPage, items: items.slice(safePage * size, safePage * size + size)};
  }
  ...
  function buildMessageRows() {
    const output = [];
    for (let index = 0; index < MESSAGE_PAGE_SIZE; index++) {
      output.push(`
      ...
      <main class="shell">
        <div class="sidebar">
          ...
          <section class="thread-section recent-section">
            ...
            <select id="recentOverflow" class="recent-overflow is-hidden" aria-label="更早的最近对话"><option value="">更早的对话…</option></select>
          </section>
        ...
        <section class="workspace">
          ...
          <section class="messages-card">
            ...
            <div id="messageList" class="message-list">${buildMessageRows()}</div>
            <div class="pager message-pager"><button id="messagePrev" class="icon-button" data-icon="chevron.left" title="较早消息">较早消息</button><span id="messagePage">1 / 1</span><button id="messageNext" class="icon-button" data-icon="chevron.right" title="较新消息">较新消息</button></div>
          </section>
  ...
  function createController(options) {
    ...
    async function render(record, suppliedState) {
      ...
      try {
        ...
        do {
          ...
          const overflowItems = recentItems.slice(RECENT_ROW_CAPACITY);
          const overflowOptions = [{value: '', label: '更早的对话…'}]
            .concat(overflowItems.map(item => ({value: item.id, label: item.title})));
          ...
          await update(record, 'recentOverflow', {
            ...
            classes: classes('recent-overflow', overflowItems.length > 0),
          });
          ...
          for (let index = 0; index < ARCHIVED_PAGE_SIZE; index++) {
            const conversation = archivedView.items[index] || null;
            const visible = !!conversation;
          ...
          const defaultMessagePage = Math.max(0, Math.ceil(messages.length / MESSAGE_PAGE_SIZE) - 1);
          if (!record.messagePages.has(state.selectedConversationId)) record.messagePages.set(state.selectedConversationId, defaultMessagePage);
          const messageView = pageSlice(messages, record.messagePages.get(state.selectedConversationId), MESSAGE_PAGE_SIZE);
          record.messagePages.set(state.selectedConversationId, messageView.page);
          await update(record, 'messageEmpty', {visible: messages.length === 0, classes: classes('message-empty', messages.length === 0)});
          await update(record, 'messagePage', {text: `${messageView.page + 1} / ${messageView.maxPage + 1}`});
          await update(record, 'messagePrev', {disabled: messageView.page <= 0, text: '较早消息'});
          await update(record, 'messageNext', {disabled: messageView.page >= messageView.maxPage, text: '较新消息'});
          for (let index = 0; index < MESSAGE_PAGE_SIZE; index++) {
            const message = messageView.items[index] || null;
    ...
    async function switchConversation(record, conversationId) {
      ...
      const current = record.session.getConversation(conversationId);
      record.messagePages.set(conversationId, Math.max(0, Math.ceil((current.messages || []).length / MESSAGE_PAGE_SIZE) - 1));
    }
    ...
    function bindWindow(record) {
      ...
      for (let index = 0; index < ARCHIVED_PAGE_SIZE; index++) {
        bind(record, record.handle.control(`archived${index}`), 'click', 'restore-conversation', async () => {
          ...
          record.titleEditing = false;
          const state = record.session.snapshot();
          const page = pageSlice(state.archived || [], record.archivedPage, ARCHIVED_PAGE_SIZE);
      ...
      bind(record, record.handle.control('messagePrev'), 'click', 'message-prev', async () => {
        const id = record.session.snapshot().selectedConversationId;
        if (!id) return;
        record.messagePages.set(id, Math.max(0, (record.messagePages.get(id) || 0) - 1));
        await render(record);
      });
      bind(record, record.handle.control('messageNext'), 'click', 'message-next', async () => {
        const id = record.session.snapshot().selectedConversationId;
        if (!id) return;
        record.messagePages.set(id, (record.messagePages.get(id) || 0) + 1);
        await render(record);
      });
      ...
      bind(record, record.handle.control('send'), 'click', 'send', async () => {
        ...
        try {
          ...
          const conversation = record.session.getConversation(id);
          record.messagePages.set(id, Math.max(0, Math.ceil((conversation.messages || []).length / MESSAGE_PAGE_SIZE) - 1));
        } catch (error) {
    ...
    function ensureWindow() {
      ...
      const task = (async () => {
        ...
        try {
          ...
          const current = session.snapshot().selectedConversation;
          if (current) record.messagePages.set(current.id, Math.max(0, Math.ceil((current.messages || []).length / MESSAGE_PAGE_SIZE) - 1));
          return record;
        } catch (error) {
          clearWindow(record);
