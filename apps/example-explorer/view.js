(function installOpenDeskExampleView(global) {
  ...
  function buildHTML(model) {
    ...
    const selectedPolicy = model.selected ? model.selected.runPolicy : '';
    return `<!doctype html><html><head><meta charset="utf-8"></head><body><div class="app">
      ...
      <div class="detail-body"><div id="overview" class="overview-grid"><div class="section"><strong class="section-title">Documentation</strong><p id="overviewDocs">${escapeHTML(model.selected && model.selected.docs ? model.selected.docs : 'No API link registered')}</p></div><div class="section"><strong class="section-title">Prerequisites</strong><p id="overviewPrerequisites">${escapeHTML(model.selected && model.selected.prerequisites.length ? model.selected.prerequisites.join('\n') : 'None declared')}</p></div><div class="section"><strong class="section-title">Expected result</strong><p id="overviewExpected">${escapeHTML(model.selected && model.selected.expected ? model.selected.expected : 'Not declared')}</p></div></div><p id="source" class="code is-hidden"></p><p id="output" class="code is-hidden">No run yet.</p></div>
      <div class="actions"><button id="run" class="primary">Run</button><button id="stop" class="stop">Stop</button><button id="copyRunCommand" title="Copy the command used to launch OpenDesk Examples">Copy Run Command</button><span class="spacer"></span><span id="runHint" class="muted">${escapeHTML(model.selected && model.selected.runnable ? 'Approved for one-click run' : 'Review prerequisites and run manually')}</span></div></section></main>
