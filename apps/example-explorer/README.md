...
- treats catalog `aliases` as historical names/search metadata, not as physical compatibility files;
...
- provides Copy Run Command for the selected canonical example;
...
- `examples/catalog.json`: canonical discovery metadata, safety policy and historical aliases.
...
A catalog entry points to exactly one canonical file. Historical names may remain in `aliases` so search and migration context are understandable, but the old files themselves are retired.
